...
.envschema.reference_data,:([]
    ...
    active:111b);
.qlog.info[`seed;"reference_data - ",.Q.s1[count .envschema.reference_data]," pairs seeded";()!()];
...
.qlog.info[`seed;"market_data - ",.Q.s1[count .envschema.market_data]," rows seeded";()!()];
...
.qlog.info[`seed;"orders/trades - ",.Q.s1[count .envschema.trades]," fills opened, one per pair in ",.Q.s1[pairs];()!()];
...
.qlog.dbg[`seed;"running: .qexec.markout_at_horizons[trades_for_markout;mo_quotes;0D00:00:00.100 0D00:00:00.500]";()!()];
.envschema.markouts,:.qexec.markout_at_horizons[trades_for_markout;mo_quotes;0D00:00:00.100 0D00:00:00.500];
.qlog.info[`seed;"markouts - ",.Q.s1[count .envschema.markouts]," horizon(s) computed for trade 1";()!()];
...
.qlog.dbg[`seed;"running: .qpos.apply_fills[.qpos.empty_book[];.envschema.trades]";()!()];
...
.qlog.info[`seed;"positions - ",.Q.s1[count pos_syms]," position(s) marked, EURUSD unrealized P&L ",.Q.s1[first exec unrealized_pnl from .envschema.positions where sym=`EURUSD];()!()];
...
.qlog.info[`seed;"ccy_exposure - net exposure computed for ",.Q.s1[count exposure]," currencies, reporting in USD";()!()];
...
.qlog.info[`seed;"predictions - one momentum_v1 signal seeded ahead of the trade";()!()];
...
.qlog.info[`seed;"order_routing/connections - all 3 orders routed to LP1, connection live";()!()];
...
.qlog.info[`seed;"economic_calendar - one upcoming high-importance EUR event seeded";()!()];

.qlog.info[`seed;"seed complete - showing every populated table";()!()];
