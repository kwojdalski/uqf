...
setup:{[]
    ...
    .qtick.schema[`tt_trade;([] time:`timestamp$(); sym:`symbol$(); px:`float$())];
    .qtick.schema[`tt_quote;([] time:`timestamp$(); sym:`symbol$(); bid:`float$())];
    };
...
test_a_schema_must_lead_with_time:{[t]
    .qtick.reset[];
    .qunit.assertThrows[.qtick.schema[`tt_x;];([] sym:`symbol$(); px:`float$());
...
test_a_schema_must_be_an_empty_unkeyed_table:{[t]
    .qtick.reset[];
    .qunit.assertThrows[.qtick.schema[`tt_x;];([] time:enlist .z.p; sym:enlist `EURUSD);
        "*must be EMPTY*";"a schema declares a shape, not data"];
    .qunit.assertThrows[.qtick.schema[`tt_x;];([sym:`symbol$()] time:`timestamp$());
...
test_subscribing_returns_the_schemas:{[t]
    setup[];
    r:.qtick.subscribe[`tt_trade;.ticktest.recorder];
...
test_subscribing_to_everything:{[t]
    setup[];
    .qunit.assertEquals[asc key .qtick.subscribe[`;.ticktest.recorder];`s#`tt_quote`tt_trade;
...
test_a_batch_reaches_only_its_own_subscribers:{[t]
    ...
    .qtick.publish[`tt_trade;([] sym:enlist `EURUSD; px:enlist 1.085)];
    .qtick.publish[`tt_quote;([] sym:enlist `EURUSD; bid:enlist 1.0849)];
...
test_subscribing_to_an_undeclared_table_is_refused:{[t]
    setup[];
    .qunit.assertThrows[.qtick.subscribe[;.ticktest.recorder];`tt_nope;
...
test_a_sink_must_be_callable:{[t]
    setup[];
    .qunit.assertThrows[.qtick.subscribe[`tt_trade;];`notafunction;
        "*must be callable*";"a symbol cannot receive a batch"]};

test_unsubscribing_stops_delivery:{[t]
    setup[];
    .qtick.subscribe[`tt_trade`tt_quote;.ticktest.recorder];
    .qunit.assertEquals[.qtick.unsubscribe .ticktest.recorder;2;"both subscriptions went"];
    .qtick.publish[`tt_trade;([] sym:enlist `EURUSD; px:enlist 1.085)];
...
test_a_keyed_batch_is_refused:{[t]
    setup[];
    .qunit.assertThrows[.qtick.publish[`tt_trade;];([sym:enlist `EURUSD] px:enlist 1.085);
        "*KEYED table*";
...
test_an_atom_column_is_refused:{[t]
    ...
    setup[];
    .qunit.assertThrows[.qtick.publish[`tt_trade;];(`EURUSD;1.085);
        "*is an ATOM*";"and it is caught at the door rather than landing sideways"]};

test_columns_may_be_published_as_a_list:{[t]
    setup[];
    .qtick.subscribe[`tt_trade;.ticktest.recorder];
    .qunit.assertEquals[.qtick.publish[`tt_trade;(enlist `EURUSD;enlist 1.085)];1;"one row went"];
...
test_a_column_list_needs_a_declared_schema:{[t]
    .qtick.reset[];
    .qunit.assertThrows[.qtick.publish[`tt_unknown;];(enlist `EURUSD;enlist 1.085);
        "*cannot name them*";
...
test_publishing_returns_the_row_count:{[t]
    setup[];
    .qunit.assertEquals[.qtick.publish[`tt_trade;([] sym:`EURUSD`GBPUSD`USDJPY; px:1.085 1.265 149.5)];3;
...
fresh_log:{[]
    ...
    `.ticktest.received set ();
    .qtick.schema[`tt_trade;([] time:`timestamp$(); sym:`symbol$(); px:`float$())];
    .qtick.open_log[log_dir[];`tt;2026.01.01];
    };

test_a_new_log_starts_empty_and_counts_what_goes_in:{[t]
    fresh_log[];
    .qtick.publish[`tt_trade;([] sym:`EURUSD`GBPUSD; px:1.085 1.265)];
    .qtick.publish[`tt_trade;([] sym:enlist `USDJPY; px:enlist 149.5)];
    .qunit.assertEquals[.qtick.msg_count;2;"two messages, not three rows - a message is a batch"]};
...
test_reopening_a_log_sees_what_is_already_in_it:{[t]
    ...
    .qtick.reset[];
    .qtick.schema[`tt_trade;([] time:`timestamp$(); sym:`symbol$(); px:`float$())];
    .qunit.assertEquals[.qtick.open_log[log_dir[];`tt;2026.01.01];1;
        "reopening finds the message already there rather than truncating it"];
    .qunit.assertEquals[.qtick.log_path;p;"and it is the same file"]};
...
test_a_plant_with_no_log_still_publishes:{[t]
    ...
    setup[];
    .qtick.subscribe[`tt_trade;.ticktest.recorder];
    .qunit.assertEquals[.qtick.publish[`tt_trade;([] sym:enlist `EURUSD; px:enlist 1.085)];1;
...
test_replay_hands_back_every_message_in_order:{[t]
    ...
    .qtick.publish[`tt_trade;([] sym:enlist `USDJPY; px:enlist 149.5)];
    `.ticktest.seen set ();
    n:.qtick.replay[.qtick.log_path;{[t;r] `.ticktest.seen set .ticktest.seen,enlist (t;count r);}];
...
test_a_replayed_batch_is_a_table_like_a_live_one:{[t]
    ...
    fresh_log[];
    .qtick.publish[`tt_trade;(enlist `EURUSD;enlist 1.085)];
    `.ticktest.shapes set ();
    .qtick.replay[.qtick.log_path;{[t;r] `.ticktest.shapes set .ticktest.shapes,type r;}];
...
test_replaying_a_log_that_is_not_there_is_not_an_error:{[t]
    .qunit.assertEquals[.qtick.replay[hsym `$"/tmp/uqf_ticktest_nonexistent";{[t;r] r}];0;
...
test_replay_restores_the_root_upd_it_found:{[t]
    ...
    .qtick.replay[.qtick.log_path;{[tb;r] }];
...
test_reset_forgets_everything:{[t]
    ...
    .qtick.subscribe[`tt_trade;.ticktest.recorder];
    .qtick.reset[];
    .qunit.assertEmpty[.qtick.subscribers;"no subscriptions"];
    .qunit.assertEmpty[.qtick.schemas;"no schemas"];
    .qunit.assertEquals[.qtick.msg_count;0;"and no message count carried over"]};
