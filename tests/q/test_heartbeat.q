...
/ which is the state .qetl.coverage.require_schema was in for a while, and the reason
/ this file ends with an integration check rather than starting with one.

\d .hbtest

setUp_fresh_heartbeat:{[] .qetl.hb.attach[]; `worker_heartbeat set 0#value `worker_heartbeat;}

/ --- shape --------------------------------------------------------------

test_attach_creates_the_declared_shape:{[t]
    .qunit.assertEquals[asc cols .qetl.hb.ledger[];asc .qetl.hb.columns;
        "the table has exactly the columns heartbeat.q declares"]};

test_require_schema_refuses_a_wrong_shape:{[t]
    / Wired into .qetl.job.bounded.init, so it can fire. A checker that cannot fire
    / protects nothing and reads as protection to anyone auditing.
    `worker_heartbeat set ([] worker:`symbol$(); nonsense:`long$());
    r:@[{.qetl.hb.require_schema[]};::;{(0b;x)}];
...
test_a_beat_records_the_state:{[t]
    .qetl.hb.beat[`w1;`running];
    .qunit.assertEquals[first exec state from .qetl.hb.report[] where worker=`w1;`running;
        "a beat records the state it was given"]};

test_a_beat_stamps_a_time:{[t]
    before:.z.p;
    .qetl.hb.beat[`w1;`running];
    seen:first exec last_seen from .qetl.hb.report[] where worker=`w1;
...
test_a_beat_does_not_count_a_window:{[t]
    ...
    .qetl.hb.beat[`w1;`running];
    .qetl.hb.beat[`w1;`running];
    .qunit.assertEquals[first exec windows from .qetl.hb.report[] where worker=`w1;0j;
        "three beats and no completed window means a window count of zero"]};

test_beat_window_increments:{[t]
    .qetl.hb.beat_window[`w1];
    .qetl.hb.beat_window[`w1];
    .qunit.assertEquals[first exec windows from .qetl.hb.report[] where worker=`w1;2j;
...
test_a_later_beat_preserves_the_window_count:{[t]
    ...
    .qetl.hb.beat_window[`w1];
    .qetl.hb.beat_window[`w1];
    .qetl.hb.beat[`w1;`completed];
    .qunit.assertEquals[first exec windows from .qetl.hb.report[] where worker=`w1;2j;
        "changing state keeps the windows already done"]};

test_one_row_per_worker:{[t]
    .qetl.hb.beat[`w1;`running];
    .qetl.hb.beat[`w1;`idle];
    .qunit.assertEquals[count select from .qetl.hb.report[] where worker=`w1;1;
...
test_stale_returns_a_table_not_names:{[t]
    ...
    .qetl.hb.beat[`w1;`running];
    .qunit.assertTrue[`state in cols .qetl.hb.stale[0D00:00:00.000000001];
...
test_has_beaten_distinguishes_never_from_stale:{[t]
    ...
    / get woken up.
    .qetl.hb.beat[`w1;`running];
    .qunit.assertEquals[.qetl.hb.has_beaten each `w1`never_ran;10b;
...
test_stale_cannot_report_a_worker_that_never_beat:{[t]
    ...
    .qunit.assertEquals[count .qetl.hb.stale[0D00:00:00.000000001];0;
...
/ never touching this table at all. That is the .qetl.coverage.require_schema state,
