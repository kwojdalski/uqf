// test_transform.q - tests for src/etl/core/transform.q (.qetl.transform) and every
...
test_every_registered_transform_passes_its_examples:{[t]
    r:select from .qetl.transform.verify_all[] where not passed;
...
/ stamps. Checked against scripts/processes/uqs_tables.q itself, read in a
...
test_a_well_formed_transform_registers:{[t]
    .qunit.assertEquals[.qetl.transform.define[`xf_mid;.xftest.decl[.xftest.mid_fn;.xftest.good_example[]]];`xf_mid;
        "a declaration with inputs, output, fn and a non-empty example registers"]};

test_a_transform_without_examples_is_refused:{[t]
    .qunit.assertError[{.qetl.transform.define[`xf_noex;x]};.xftest.decl[.xftest.mid_fn;()];
...
test_a_transform_whose_examples_are_all_empty_is_refused:{[t]
    ...
    .qunit.assertError[{.qetl.transform.define[`xf_empty;x]};.xftest.decl[.xftest.mid_fn;ex];
...
test_a_keyed_output_is_refused:{[t]
    ...
    .qunit.assertError[{.qetl.transform.define[`xf_keyed;x]};d;"a keyed table cannot be published to a tickerplant"]};

test_an_fn_of_the_wrong_arity_is_refused:{[t]
    .qunit.assertError[{.qetl.transform.define[`xf_arity;x]};.xftest.decl[{[a;b] a};.xftest.good_example[]];
...
test_an_example_of_the_wrong_shape_is_refused_at_declaration:{[t]
    ...
    .qunit.assertError[{.qetl.transform.define[`xf_badex;x]};.xftest.decl[.xftest.mid_fn;ex];
...
test_an_as_of_transform_needs_an_instant_in_every_example:{[t]
    ...
    d[`as_of]:1b;
    .qunit.assertError[{.qetl.transform.define[`xf_clock;x]};d;
...
test_apply_returns_the_output:{[t]
    ...
    .qunit.assertEquals[.qetl.transform.apply[`xf_mid;enlist[`quotes]!enlist .xftest.two_quotes];.xftest.two_mids;
        "apply runs fn over the named inputs"]};

test_apply_refuses_an_input_of_the_wrong_type:{[t]
    .qetl.transform.define[`xf_mid;.xftest.decl[.xftest.mid_fn;.xftest.good_example[]]];
    .qunit.assertError[{.qetl.transform.apply[`xf_mid;x]};enlist[`quotes]!enlist update bid:1 2 from .xftest.two_quotes;
...
test_apply_refuses_an_input_with_an_extra_column:{[t]
    .qetl.transform.define[`xf_mid;.xftest.decl[.xftest.mid_fn;.xftest.good_example[]]];
    .qunit.assertError[{.qetl.transform.apply[`xf_mid;x]};enlist[`quotes]!enlist update venue:`a`b from .xftest.two_quotes;
        "the inputs say exactly what a transform reads"]};

test_apply_refuses_a_missing_input:{[t]
    .qetl.transform.define[`xf_mid;.xftest.decl[.xftest.mid_fn;.xftest.good_example[]]];
    .qunit.assertError[{.qetl.transform.apply[`xf_mid;x]};enlist[`other]!enlist .xftest.two_quotes;
...
test_a_clocked_transform_must_be_applied_with_an_instant:{[t]
    ...
    d[`as_of]:1b;
    .qetl.transform.define[`xf_clock;d];
    .qunit.assertError[{.qetl.transform.apply[`xf_clock;x]};enlist[`quotes]!enlist .xftest.two_quotes;
...
test_verify_reports_a_wrong_expected_value_with_the_value:{[t]
    ...
    .qetl.transform.define[`xf_wrong;.xftest.decl[.xftest.mid_fn;ex]];
    r:.qetl.transform.verify `xf_wrong;
...
test_verify_catches_a_clock_in_the_output:{[t]
    ...
    .qetl.transform.define[`xf_now;d];
    .qunit.assertEquals[first (.qetl.transform.verify `xf_now)`passed;0b;
...
test_verify_catches_output_that_changes_between_calls:{[t]
    ...
    .qetl.transform.define[`xf_drift;.xftest.decl[fn;.xftest.good_example[]]];
    r:first .qetl.transform.verify `xf_drift;
...
test_verify_runs_the_empty_case_every_transform_gets:{[t]
    ...
    .qetl.transform.define[`xf_noempty;.xftest.decl[fn;.xftest.good_example[]]];
    r:.qetl.transform.verify `xf_noempty;
...
test_float_columns_compare_within_tolerance:{[t]
    ...
    .qetl.transform.define[`xf_close;.xftest.decl[.xftest.mid_fn;ex]];
    .qunit.assertEquals[all (.qetl.transform.verify `xf_close)`passed;1b;
...
test_float_columns_differing_beyond_tolerance_do_not_match:{[t]
    ...
    .qetl.transform.define[`xf_far;.xftest.decl[.xftest.mid_fn;ex]];
    .qunit.assertEquals[first (.qetl.transform.verify `xf_far)`passed;0b;
...
test_require_verified_names_the_failing_example:{[t]
    ...
    .qetl.transform.define[`xf_req;.xftest.decl[.xftest.mid_fn;ex]];
    err:@[.qetl.transform.require_verified;`xf_req;{x}];
...
test_a_passthrough_returns_its_input:{[t]
    .qetl.transform.passthrough[`xf_pass;`batch;.xftest.quotes_schema;.xftest.two_quotes];
    .qunit.assertEquals[.qetl.transform.apply[`xf_pass;enlist[`batch]!enlist .xftest.two_quotes];.xftest.two_quotes;
