// test_coercion.q - tests for src/etl/core/coercion.q (.qetl.coerce), the shared
// text-to-type layer.
//
...
test_a_comma_decimal_is_recovered:{[t]
    .qunit.assertEquals[.qetl.coerce.to_float["1,0842"];1.0842;"a European decimal separator survives"]};

test_a_plain_decimal_still_works:{[t]
    .qunit.assertEquals[.qetl.coerce.to_float["1.0842"];1.0842;"the ordinary form is unaffected"]};
...
test_a_negative_parses:{[t]
    .qunit.assertEquals[.qetl.coerce.to_float["-0.5"];-0.5;"a negative is not malformed"]};

test_text_that_is_not_a_number_is_nulled:{[t]
    .qunit.assertEquals[null .qetl.coerce.to_float["n/a"];1b;"a non-numeric field is null rather than a partial parse"]};
...
test_empty_is_null_not_zero:{[t]
    .qunit.assertEquals[null .qetl.coerce.to_float[""];1b;"a missing number must never read as a real zero"]};

test_whitespace_only_is_null:{[t]
    .qunit.assertEquals[null .qetl.coerce.to_float["   "];1b;"a padded empty field is still empty"]};

test_a_long_parses_and_empties_to_null:{[t]
    .qunit.assertEquals[(.qetl.coerce.to_long["1000000"];null .qetl.coerce.to_long[""]);(1000000j;1b);"the same empty rule applies to integers"]};
...
test_a_date_only_value_is_refused_not_widened:{[t]
    .qunit.assertEquals[null .qetl.coerce.to_timestamp["2026-09-15"];1b;"refusing beats destroying intraday ordering"]};

test_a_date_only_q_format_value_is_also_refused:{[t]
    .qunit.assertEquals[null .qetl.coerce.to_timestamp["2026.09.15"];1b;"the refusal is about the missing time, not the separator style"]};

test_an_iso_timestamp_parses:{[t]
    .qunit.assertEquals[.qetl.coerce.to_timestamp["2026-09-15T09:30:00"];2026.09.15D09:30:00.000000000;"ISO is a legitimate source format"]};

test_a_q_format_timestamp_parses:{[t]
    .qunit.assertEquals[.qetl.coerce.to_timestamp["2026.09.15D09:30:00"];2026.09.15D09:30:00.000000000;"so is q's own"]};

test_sub_second_precision_survives:{[t]
    .qunit.assertEquals[.qetl.coerce.to_timestamp["2026-09-15T09:30:00.123456789"];2026.09.15D09:30:00.123456789;"nanosecond precision is not rounded away"]};
...
test_a_malformed_timestamp_is_null:{[t]
    .qunit.assertEquals[null .qetl.coerce.to_timestamp["not a date"];1b;"garbage is null rather than a partial parse"]};
...
test_case_is_folded:{[t]
    .qunit.assertEquals[.qetl.coerce.to_symbol[" eurusd"];`EURUSD;"the library's convention is upper-case pairs throughout"]};
...
test_the_trim_is_still_applied:{[t]
    .qunit.assertEquals[.qetl.coerce.to_symbol["EURUSD "];`EURUSD;"defensive rather than corrective, but consistent"]};

test_case_can_be_preserved_when_it_matters:{[t]
    .qunit.assertEquals[.qetl.coerce.to_symbol_cased[" AbC123 "];`AbC123;"an exchange order id is case-sensitive, and still gets trimmed"]};

test_an_empty_symbol_is_the_null_symbol:{[t]
    .qunit.assertEquals[.qetl.coerce.to_symbol[""];` ;"absent is distinguishable from a real value"]};
...
test_a_mandatory_null_is_refused:{[t]
    .qunit.assertError[{.qetl.coerce.require_present[`rate;x]};0n;"a trade with no rate must not be published"]};

test_a_present_value_passes_through:{[t]
    .qunit.assertEquals[.qetl.coerce.require_present[`rate;1.0842];1.0842;"a real value is returned unchanged"]};

test_the_refusal_names_the_column:{[t]
    err:@[{.qetl.coerce.require_present[`rate;x]; ""};0n;{x}];
...
test_a_column_reports_its_failure_count:{[t]
    r:.qetl.coerce.coerce_column[.qetl.coerce.to_float;("1.0842";"";"bad";"1.2631")];
...
test_a_wholly_failed_column_is_visible:{[t]
    r:.qetl.coerce.coerce_column[.qetl.coerce.to_timestamp;("2026-09-15";"2026-09-16")];
...
test_a_clean_column_reports_no_failures:{[t]
    r:.qetl.coerce.coerce_column[.qetl.coerce.to_float;("1.0842";"1,2631")];
