...
test_a_null_source_version_is_refused_at_init:{[t]
    .qunit.assertError[{.qddbf.init x};.ddbftest.spec_for[`;1;4];"a run that cannot name its release cannot record coverage (E-09)"]};
...
test_init_takes_the_single_instance_lock:{[t]
    .qddbf.init[.ddbftest.spec_for[`v1;1;4]];
    .qunit.assertEquals[.qbfstate.lock_held `demo_deals_backfill;1b;"one instance per worker is what keeps the checkpoint private (E-06)"]};
...
/ E-10, in the direction that matters: a version bump exists to force
...
/ A retry after a partial run redoes only the gap. This is the case E-13
...
/ --- contract validation on the fetched rows (E-12) ---------------------
