...
test_side_levels_takes_a_book_side:{[t]
    ...
    rows:0!state[];
    .qunit.assertEquals[(.qpipe.job.superbook.side_levels[rows;`bid])`price;1.101 1.099 1.098 1.097;"`bid is the bids, best first"];
    .qunit.assertEquals[(.qpipe.job.superbook.side_levels[rows;`ask])`price;1.100 1.102 1.103 1.104;"`ask is the asks, best first"];
    {[r;s] .qunit.assertThrows[.qpipe.job.superbook.side_levels[r;];s;
...
test_update_replaces_a_source_instead_of_accumulating_history:{[t]
    ...
    s:.qpipe.job.superbook.replace_books[state[];newer;d 1];
...
test_out_of_order_and_replayed_rows_do_not_rewind_the_book:{[t]
    ...
    s:.qpipe.job.superbook.replace_books[state[];newer;d 2];
    replay:.qpipe.job.superbook.replace_books[s;fixtures[];d 3];
...
test_equal_timestamp_uses_last_arrival:{[t]
    ...
    s:.qpipe.job.superbook.replace_books[state[];newer;d 0];
...
test_full_withdrawal_clears_and_keeps_the_timestamp_watermark:{[t]
    ...
    s:.qpipe.job.superbook.replace_books[state[];withdrawal;d 2];
    s:.qpipe.job.superbook.replace_books[s;fixtures[];d 3];
...
test_zero_null_negative_and_infinite_levels_are_not_executable:{[t]
    r:.qpipe.job.superbook.levels[1 2 0 0n 0w 6 7 8 9f;10 0 20 20 20 -1 0n 0w 30f];
...
test_zero_size_update_does_not_leave_old_liquidity:{[t]
    ...
    s:.qpipe.job.superbook.replace_books[state[];zero;d 1];
...
test_stale_sources_expire_independently_and_all_stale_pairs_clear:{[t]
    ...
    s:.qpipe.job.superbook.replace_books[state[];newer;d 4];
...
test_future_rows_cannot_poison_the_watermark:{[t]
    ...
    s:.qpipe.job.superbook.replace_books[state[];future;d 1];
...
test_pairs_and_inverse_pairs_are_kept_separate:{[t]
    ...
    s:.qpipe.job.superbook.replace_books[empty[];rows;d 0];
    r:snapshot[s;0];
...
test_locked_and_uncrossed_books_have_no_opportunity:{[t]
    ...
    s:.qpipe.job.superbook.replace_books[state[];rows;d 0];
    r:arb[s;0];
...
test_single_source_cross_is_not_cross_source_arb:{[t]
    ...
    s:.qpipe.job.superbook.replace_books[s;1_fixtures[];d 0];
    r:arb[s;0];
...
test_malformed_vectors_are_named:{[t]
    .qunit.assertThrows[{.qpipe.job.superbook.levels[x;100 200f]};enlist 1.1;
        "*differ in length*";"mismatched vectors cannot misattribute quantity"];
    .qunit.assertThrows[{.qpipe.job.superbook.levels[x;100 200f]};"ab";
        "*numeric vectors*";"text prices are not silently cast"]};

test_missing_identity_and_timestamp_are_refused:{[t]
    .qunit.assertThrows[{.qpipe.job.superbook.replace_books[.sbtest.empty[];x;.sbtest.d 0]};delete source from fixtures[];
        "*missing required column(s) source*";"source identity is mandatory"];
    .qunit.assertThrows[{.qpipe.job.superbook.replace_books[.sbtest.empty[];x;.sbtest.d 0]};update source:` from fixtures[];
        "*non-null source*";"blank sources cannot be pooled"];
    .qunit.assertThrows[{.qpipe.job.superbook.replace_books[.sbtest.empty[];x;.sbtest.d 0]};update source_time:0Np from fixtures[];
...
test_normalizer_keeps_multiple_quote_sources_and_filters_equities:{[t]
    ...
    r:.qetl.job.stream.normalizer.normalize[`market_data;`quote;rows];
...
forward:{[t;x]
    record[t;x];
    if[t=`market_data; .qpipe.job.superbook.on_batch[t;x]];
    if[t=`superbook; .qpipe.job.arbitrage.on_batch[t;x]];
    }
...
test_live_handlers_and_timer_publish_opportunities_then_clears:{[t]
    ...
    .qetl.job.stream.wire[`market_data;forward];
    .qetl.job.stream.wire[`superbook;forward];
    .qetl.job.stream.wire[`arbitrage;record];
    rows:([] time:2#.z.p;sym:`EURUSD`EURUSD;bid:1.101 1.099;
        ask:1.103 1.100;bsize:100 500;asize:200 60;src:`LP_A`LP_B);
    .qpipe.job.market_data.on_batch[`quote;rows];
    ...
    `.qpipe.job.superbook.books set `sym`source xkey update source_time:.z.p-0D00:00:10 from 0!.qpipe.job.superbook.books;
    ...
    .qunit.assertEquals[cols out;cols .qpipe.job.arbitrage.arbitrage;"publisher sends declared columns without time"]};
...
test_unknown_tables_and_empty_batches_publish_nothing:{[t]
    ...
    .qetl.job.stream.wire[`arbitrage;record];
    .qpipe.job.superbook.on_batch[`unrelated;fixtures[]];
    .qpipe.job.arbitrage.on_batch[`unrelated;.qpipe.job.superbook.superbook];
    .qpipe.job.arbitrage.on_batch[`superbook;.qpipe.job.superbook.superbook];
    .qpipe.job.superbook.on_timer[];
...
test_malformed_batch_does_not_partly_update_live_state:{[t]
    ...
    .qunit.assertThrows[{.qpipe.job.superbook.on_batch[`market_data;x]};rows;
        "*differ in length*";"a malformed second row fails the whole batch"];
    .qunit.assertEquals[.qpipe.job.superbook.books;state[];"the first row was not committed before validation finished"]};
...
test_tickerplant_routes_all_three_processes_using_the_real_schemas:{[t]
    ...
    {[job]
        ...
        .qetl.job.stream.wire[job;.qetl.tick.publish];
        d:.qetl.job.stream.def job;
        .qetl.tick.subscribe[d`subscribe_to;{[handler;msg] handler . 1_msg}[d`on_batch]];
        } each `market_data`superbook`arbitrage;
    .qetl.tick.subscribe[enlist `arbitrage;{[msg] .sbtest.record[msg 1;msg 2]}];
    .qetl.tick.publish[`quote;([] sym:`EURUSD`EURUSD;bid:1.101 1.099;
        ask:1.103 1.100;bsize:100 500;asize:200 60;src:`LP_A`LP_B)];
    out:first last published`rows;
