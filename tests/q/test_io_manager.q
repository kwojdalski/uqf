...
setUp_clean:{[]
    ...
    `.qio.touched set 0#.qio.touched;
    `.qio.default set .qio.memory;
    }
...
test_a_config_without_io_gets_the_process_default:{[t]
    ...
    `.qio.default set .qio.discard;
    .qunit.assertEquals[.qio.for_cfg[`source`dataset`width!(`s;`d;1D)];.qio.discard;
        "a worker that declares no manager gets whatever the process chose"]};

test_a_declared_manager_beats_the_process_default:{[t]
    `.qio.default set .qio.discard;
    cfg:`source`dataset`width`io!(`s;`d;1D;.qio.memory);
    .qunit.assertEquals[.qio.for_cfg cfg;.qio.memory;"the worker's own choice wins"]};

test_finish_is_a_no_op_for_a_manager_without_one:{[t]
    .qunit.assertEquals[.qio.finish .qio.memory;::;"memory has nothing to do at the end of a run"]};

test_a_finish_that_is_not_a_function_is_refused:{[t]
    .qunit.assertThrows[{.qio.require_manager x};`write`finish!({[t;b] count b};42);
...
test_hdb_writes_each_row_into_its_own_date:{[t]
    root:hdb_dir[];
    .qio.write[.qio.hdb[root;`deal_time];`iodeals;deals[]];
...
test_hdb_partitions_by_time_when_the_batch_has_one:{[t]
    ...
    .qio.write[.qio.hdb[root;`not_a_column];`iobook;b];
...
test_hdb_appends_a_second_window:{[t]
    root:hdb_dir[];
    m:.qio.hdb[root;`deal_time];
    .qio.write[m;`iodeals;deals[]];
    .qio.write[m;`iodeals;deals[]];
...
test_hdb_refuses_today:{[t]
    ...
    .qunit.assertThrows[{.qio.write[.qio.hdb[x;`deal_time];`iodeals;y]}[root];b;
...
test_hdb_refuses_a_batch_with_nothing_to_partition_by:{[t]
    ...
    .qunit.assertThrows[{[r;ignored] .qio.write[.qio.hdb[r;`deal_time];`iodeals;([] sym:enlist `EURUSD)]}[root];::;
...
test_hdb_finish_fills_a_partition_missing_a_table:{[t]
    ...
    m:.qio.hdb[root;`deal_time];
    .qio.write[m;`iodeals;deals[]];
    .qio.write[m;`iotrades;([] deal_time:enlist 2026.01.03D12:00:00.000000000; sym:enlist `EURUSD)];
    .qio.finish m;
...
test_hdb_appends_to_a_finished_partition:{[t]
    ...
    .qio.write[m;`iodeals;deals[]];
    .qio.finish m;
    .qio.write[m;`iodeals;([] deal_time:enlist 2026.01.02D08:00:00.000000000; sym:enlist `AUDUSD; notional:enlist 5e6)];
    .qio.finish m;
    p:part[root;2026.01.02;`iodeals];
...
test_hdb_finish_forgets_what_it_finished:{[t]
    ...
    .qio.write[m;`iodeals;deals[]];
    .qio.finish m;
    .qunit.assertEquals[.qio.finish m;0;"a second finish has nothing left to do"]};

test_hdb_refuses_a_root_that_is_not_a_file_symbol:{[t]
    .qunit.assertThrows[{.qio.hdb[x;`deal_time]};`plain;
