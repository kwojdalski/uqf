...
beforeNamespace_load:{[]
    `.cfgatest.saved_watched set .qetl.cfg.audit.watched;
    `.cfgatest.saved_seen set .qetl.cfg.audit.seen;
    }

afterNamespace_restore:{[]
    `.qetl.cfg.audit.watched set .cfgatest.saved_watched;
    `.qetl.cfg.audit.seen set .cfgatest.saved_seen;
    }

reset:{[]
    `.qetl.cfg.audit.watched set (`symbol$())!();
    .qetl.cfg.audit.forget[];
...
test_a_bare_name_is_refused:{[t]
    ...
    .qunit.assertThrows[{.qetl.cfg.audit.watch[`j;x]};`a_number;
        "*fully qualified*";
        "an unqualified name would silently audit the wrong variable"]};

test_registering_twice_does_not_duplicate:{[t]
    reset[];
    .qetl.cfg.audit.watch[`j;`.cfgatest.a_number];
    .qetl.cfg.audit.watch[`j;`.cfgatest.a_number`.cfgatest.a_span];
    .qunit.assertEquals[asc .qetl.cfg.audit.watching[`j];
...
test_an_owner_that_declares_nothing_watches_nothing:{[t]
    reset[];
    .qunit.assertEquals[count .qetl.cfg.audit.watching[`never_registered];0;
...
test_the_first_poll_records_the_starting_value:{[t]
    reset[];
    .qetl.cfg.audit.watch[`j;`.cfgatest.a_number];
    r:first .qetl.cfg.audit.poll[`j;t0];
...
test_an_unchanged_value_is_not_reported_again:{[t]
    ...
    .qunit.assertEquals[count .qetl.cfg.audit.poll[`j;t0+0D00:00:05];0;
...
test_only_the_named_owner_is_polled:{[t]
    reset[];
    .qetl.cfg.audit.watch[`mine;`.cfgatest.a_number];
    .qetl.cfg.audit.watch[`theirs;`.cfgatest.a_span];
    r:.qetl.cfg.audit.poll[`mine;t0];
...
test_an_undefined_name_is_recorded_rather_than_thrown:{[t]
    reset[];
    .qetl.cfg.audit.watch[`j;`.cfgatest.nothing.is.here];
    r:first .qetl.cfg.audit.poll[`j;t0];
...
test_a_name_that_appears_later_is_reported_as_a_change:{[t]
    reset[];
    .qetl.cfg.audit.watch[`j;`.cfgatest.appears_later];
    .qetl.cfg.audit.poll[`j;t0];
...
test_the_timer_target_is_a_function_not_the_result_of_calling_one:{[t]
    reset[];
    .qunit.assertTrue[100h=type .qetl.cfg.audit.poll_and_publish;
        "a timer target that is not a lambda runs once at wiring time and never again"]};

test_the_timer_target_is_niladic:{[t]
    / .qtorq.safe_timer wraps it as `@[f;::;handler]`, the idiom invariant 4
    ...
    .qunit.assertEquals[(value .qetl.cfg.audit.poll_and_publish)1;enlist `;
        "safe_timer calls it with no arguments"]};

test_nothing_is_published_before_the_runner_names_this_processs_job:{[t]
    reset[];
    `.qetl.cfg.audit.owner_here set `;
    .qunit.assertEquals[.qetl.cfg.audit.poll_and_publish[];();
...
test_the_publisher_sends_through_the_jobs_own_seam_when_called:{[t]
    reset[];
    `.cfgatest.sent set ();
    .qetl.cfg.audit.watch[`cross_arbitrage;`.cfgatest.a_number];
    `.qetl.cfg.audit.owner_here set `cross_arbitrage;
    .qetl.job.stream.wire[`cross_arbitrage;{[t;x] `.cfgatest.sent set (t;x)}];
...
test_the_watched_names_exist:{[t]
    ...
    reset[];
    `.qetl.cfg.audit.watched set .cfgatest.saved_watched;
    declared:raze value .qetl.cfg.audit.watched;
...
test_every_watching_job_declares_config_change_as_an_output:{[t]
    ...
    reset[];
    `.qetl.cfg.audit.watched set .cfgatest.saved_watched;
    owners:key .qetl.cfg.audit.watched;
    owners:owners where owners in .qetl.job.stream.defined[];
    bad:owners where not {[j] `config_change in (.qetl.job.stream.def j)`publishes} each owners;
