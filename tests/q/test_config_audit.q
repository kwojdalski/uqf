...
beforeNamespace_load:{[]
    `.cfgatest.saved_watched set .qaudit.watched;
    `.cfgatest.saved_seen set .qaudit.seen;
    }

afterNamespace_restore:{[]
    `.qaudit.watched set .cfgatest.saved_watched;
    `.qaudit.seen set .cfgatest.saved_seen;
    }

reset:{[]
    `.qaudit.watched set (`symbol$())!();
    .qaudit.forget[];
...
test_a_bare_name_is_refused:{[t]
    ...
    .qunit.assertThrows[{.qaudit.watch[`j;x]};`a_number;
        "*fully qualified*";
        "an unqualified name would silently audit the wrong variable"]};

test_registering_twice_does_not_duplicate:{[t]
    reset[];
    .qaudit.watch[`j;`.cfgatest.a_number];
    .qaudit.watch[`j;`.cfgatest.a_number`.cfgatest.a_span];
    .qunit.assertEquals[asc .qaudit.watching[`j];
...
test_an_owner_that_declares_nothing_watches_nothing:{[t]
    reset[];
    .qunit.assertEquals[count .qaudit.watching[`never_registered];0;
...
test_an_unchanged_value_is_not_reported_again:{[t]
    ...
    .qaudit.watch[`j;`.cfgatest.a_number];
    .qaudit.poll[`j;t0];
    .qunit.assertEquals[count .qaudit.poll[`j;t0+0D00:00:05];0;
...
test_only_the_named_owner_is_polled:{[t]
    reset[];
    .qaudit.watch[`mine;`.cfgatest.a_number];
    .qaudit.watch[`theirs;`.cfgatest.a_span];
    r:.qaudit.poll[`mine;t0];
...
test_an_undefined_name_is_recorded_rather_than_thrown:{[t]
    reset[];
    .qaudit.watch[`j;`.cfgatest.nothing.is.here];
    r:first .qaudit.poll[`j;t0];
...
test_a_name_that_appears_later_is_reported_as_a_change:{[t]
    reset[];
    .qaudit.watch[`j;`.cfgatest.appears_later];
    .qaudit.poll[`j;t0];
...
test_the_timer_target_is_a_function_not_the_result_of_calling_one:{[t]
    reset[];
    .qunit.assertTrue[100h=type .qaudit.poll_and_publish;
...
test_the_timer_target_is_niladic:{[t]
    ...
    .qunit.assertEquals[(value .qaudit.poll_and_publish)1;enlist `;
        "safe_timer calls it with no arguments"]};

test_nothing_is_published_before_the_runner_names_this_processs_job:{[t]
    reset[];
    `.qaudit.owner_here set `;
    .qunit.assertEquals[.qaudit.poll_and_publish[];();
...
test_the_publisher_sends_through_the_jobs_own_seam_when_called:{[t]
    reset[];
    `.cfgatest.sent set ();
    .qaudit.watch[`cross_arbitrage;`.cfgatest.a_number];
    `.qaudit.owner_here set `cross_arbitrage;
...
test_the_watched_names_exist:{[t]
    ...
    reset[];
    `.qaudit.watched set .cfgatest.saved_watched;
    declared:raze value .qaudit.watched;
