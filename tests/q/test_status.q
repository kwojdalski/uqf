...
write:{[state;err] .qstatus.write_status[`w;`st1;state;.statustest.spec[];.statustest.progress[];err]}
...
test_a_reversed_range_is_refused:{[t]
    ...
    .qunit.assertError[{.qstatus.write_status[`w;`st1;`running;x;.statustest.progress[];""]};bad;"a bad range can never reach the file (ETL-08)"]};

test_a_null_source_version_is_refused:{[t]
    bad:`source_version`range_from`range_to!(`;.statustest.d 1;.statustest.d 2);
    .qunit.assertError[{.qstatus.write_status[`w;`st1;`running;x;.statustest.progress[];""]};bad;"source_version is mandatory (ETL-09)"]};

/ --- reading the previous state --------------------------------------------

test_no_file_means_no_previous_state:{[t]
    .qunit.assertEquals[null .qstatus.previous_state `st1;1b;"a first write has nothing to transition from"]};
...
test_starting_may_begin_running:{[t]
    .qunit.assertEquals[.qstatus.require_transition[`starting;`running];1b;"the ordinary path"]};

/ running -> running is the next window, not a stutter.
test_running_may_continue_running:{[t]
    .qunit.assertEquals[.qstatus.require_transition[`running;`running];1b;"each window reports progress under the same state"]};

test_running_may_complete:{[t]
    .qunit.assertEquals[.qstatus.require_transition[`running;`completed];1b;"a finished run is terminal"]};
...
test_a_completed_run_may_not_go_back_to_running:{[t]
    .qunit.assertError[{.qstatus.require_transition[`completed;x]};`running;"resurrecting a completed run hides whatever happens next"]};

test_a_failed_run_may_not_report_completed:{[t]
    .qunit.assertError[{.qstatus.require_transition[`failed;x]};`completed;"a failure overwritten by success is the worst possible silent outcome"]};

test_an_idle_run_may_not_go_back_to_running:{[t]
    .qunit.assertError[{.qstatus.require_transition[`idle;x]};`running;"idle is terminal for that run"]};
...
test_every_terminal_state_may_start_a_new_run:{[t]
    .qunit.assertEquals[
        .qstatus.require_transition[;`starting] each `idle`completed`failed;
        111b;
...
test_a_failure_is_recordable_from_any_state:{[t]
    .qunit.assertEquals[
        .qstatus.require_transition[;`failed] each `starting`running`idle`completed`failed;
        11111b;
...
test_an_unknown_previous_state_is_refused:{[t]
    .qunit.assertError[{.qstatus.require_transition[x;`running]};`nonsense;"a state nobody defined cannot be reasoned about"]};
...
test_the_refusal_explains_what_it_prevents:{[t]
    err:@[{.qstatus.require_transition[`failed;x]; ""};`completed;{x}];
...
test_a_refused_write_leaves_the_recorded_status_alone:{[t]
    ...
    .qunit.assertEquals[.qstatus.previous_state `st1;`failed;"the failure survives an attempt to overwrite it with success"]};
...
test_the_ordinary_lifecycle_writes_end_to_end:{[t]
    ...
    .statustest.write[`running;""];
    .statustest.write[`completed;""];
    .qunit.assertEquals[.qstatus.previous_state `st1;`completed;"start, two windows, finish"]};
