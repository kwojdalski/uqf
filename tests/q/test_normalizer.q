...
/ .qetl.job.stream.define refuses a second job on one procname, and a test that
...
forget:{[name]
    `.qetl.job.stream.normalizer.registry set (enlist name) _ .qetl.job.stream.normalizer.registry;
    `.qetl.job.stream.jobs set (enlist name) _ .qetl.job.stream.jobs;
    pn:key[.qetl.job.stream.procnames] where name=value .qetl.job.stream.procnames;
    `.qetl.job.stream.procnames set pn _ .qetl.job.stream.procnames;
    }

define_ok:{[name;pn]
    mk_xf[`normtest_good;canon;good];
    (` sv `.qpipe.job,name,`publish) set .qetl.job.stream.unwired name;
    .qetl.job.stream.normalize[name;`procname`output`input!(pn;canon;(enlist `src)!enlist `normtest_good)]}

/ ------------------------------------------------------------- DEFINING

test_define_registers_the_job_with_its_edges_derived:{[t]
    define_ok[`nt_a;`nta1];
    d:.qetl.job.stream.def `nt_a;
...
test_an_output_with_time_is_refused:{[t]
    .qunit.assertThrows[.qetl.job.stream.normalize[`nt_c;];
...
test_an_unregistered_transform_is_refused:{[t]
    .qunit.assertThrows[.qetl.job.stream.normalize[`nt_d;];
...
test_a_mapping_whose_output_drifts_is_refused:{[t]
    ...
    .qunit.assertThrows[.qetl.job.stream.normalize[`nt_e;];
    ...
    .qunit.assertThrows[.qetl.job.stream.normalize[`nt_f;];
...
test_a_mapping_with_two_inputs_is_refused:{[t]
    .qetl.transform.define[`normtest_two;`inputs`output`fn`examples!(
        `p`q!(([] x:`long$());([] y:`long$()));
        ([] a:`long$(); b:`symbol$());
        {[p;q] ([] a:p`x; b:(count p)#`z)};
        enlist `inputs`expected!(`p`q!(([] x:enlist 1);([] y:enlist 2));([] a:enlist 1; b:enlist `z)))];
    .qunit.assertThrows[.qetl.job.stream.normalize[`nt_g;];
...
test_dispatch_publishes_through_the_instances_own_seam:{[t]
    define_ok[`nt_k;`ntk1];
    `.normtest.got set ();
    .qetl.job.stream.wire[`nt_k;{[t;x] `.normtest.got set .normtest.got,enlist (t;x); count x}];
...
test_dispatch_publishes_nothing_for_an_empty_batch:{[t]
    define_ok[`nt_l;`ntl1];
    `.normtest.got set ();
    .qetl.job.stream.wire[`nt_l;{[t;x] `.normtest.got set .normtest.got,enlist (t;x); count x}];
...
test_the_shipped_normalizers_are_defined_and_registered:{[t]
    .qunit.assertEquals[`executions`marks in .qetl.job.stream.normalizer.defined[];11b;"executions and marks are defined"];
    .qunit.assertEquals[.qetl.job.stream.def[`executions]`subscribe_to;`trades`crypto_trades;
        "executions reads both fill tables"];
    .qunit.assertEquals[.qetl.job.stream.def[`marks]`subscribe_to;`quote`crypto_book;
...
test_every_shipped_mapping_verifies:{[t]
    ...
    xfs:raze value each (.qetl.job.stream.normalizer.def[`executions]`input;.qetl.job.stream.normalizer.def[`marks]`input);
    failed:select from raze .qetl.transform.verify each xfs where not passed;
