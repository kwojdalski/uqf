...
/ (.egtest).
/ .
/ I-08's other half. There are 113 `@eg` lines under src/ carrying a `->`
/ expected value, and until this file NOTHING RAN ANY OF THEM. They are an
/ assertion suite with no runner: 113 claims about what this library returns,
...
/ .
/ WHAT THIS ASSERTS, and what it deliberately does not. Zero MISMATCHES is
/ the invariant: no documented value may be wrong. Examples that cannot be
/ evaluated standalone (twelve of them need a `tape`, `quotes` or `requests`
/ table in scope) are reported and tolerated, because inventing a fixture for
/ each would be a second, unreviewed copy of the data the real tests use.
/ Three expectations are prose rather than values; they are counted as prose
/ rather than quietly passed, so a value that stops parsing cannot hide there.
...
results:{[] classify each assertions[]}
...
test_most_examples_are_actually_evaluated:{[t]
    ...
    / well below the 98 that pass today.
    st:results[];
    .qunit.assertTrue[80<=sum st=`pass;
