...
/ .
/   subscribeto  it receives rows as the plant delivers them, `time` first
...
job_tables:{[]
    raze {[job]
        ...
        ([] job:(count nms)#job; tbl:nms;
            role:{[d;nm] $[nm in d`subscribeto;`subscribeto;nm in d`publishes;`publishes;`internal]}[d] each nms)
