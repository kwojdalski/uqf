...
test_markout_known_single_buy:{[t] .testutil.assertApprox[.qf.markout[1;1.1000;1.1010;10000];10f;1e-6;"buy, price rallies 10 pips after -> +10 pip markout"]};
test_markout_known_single_sell:{[t] .testutil.assertApprox[.qf.markout[-1;1.1000;1.0990;10000];10f;1e-6;"sell, price falls 10 pips after -> +10 pip markout (favourable)"]};
test_markout_adverse_move:{[t] .testutil.assertApprox[.qf.markout[1;1.1000;1.0990;10000];-10f;1e-6;"buy, price falls after -> negative (adverse) markout"]};
test_markout_zero_when_flat:{[t] .testutil.assertApprox[.qf.markout[1;1.1000;1.1000;10000];0f;1e-9;"no move -> zero markout"]};

test_markout_vectorized_across_horizons:{[t]
    horizons:1.1010 1.1005 1.0995 1.1000;
    expected:10 5 -5 0;
    .testutil.assertApprox[.qf.markout[1;1.1000;horizons;10000];expected;1e-6;"markout profile across several post-trade horizons"]};

test_markout_side_flip_is_negation:{[t]
    buy_side:.qf.markout[1;1.1000;1.1010;10000];
    sell_side:.qf.markout[-1;1.1000;1.1010;10000];
...
test_markout_at_horizons_known_values:{[t]
    ...
    r:.qf.markout_at_horizons[trades;quotes;0D00:00:01 0D00:00:10];
...
test_markout_at_horizons_finds_most_recent_quote_before_target:{[t]
    ...
    r:.qf.markout_at_horizons[trades;quotes;0D00:00:10];
...
test_markout_at_horizons_matches_direct_markout_call:{[t]
    ...
    / markout_pips must equal a direct .qf.markout call with that ref_price
    trades:([] sym:enlist `EURUSD; time:enlist 2024.01.01D09:00:00.000000000; side:enlist 1; trade_price:enlist 1.1000; pip_factor:enlist 10000);
    quotes:([] sym:enlist `EURUSD; time:enlist 2024.01.01D09:00:01.000000000; mid:enlist 1.1023);
    r:.qf.markout_at_horizons[trades;quotes;0D00:00:05];
    direct_result:.qf.markout[1;1.1000;first r`ref_price;10000];
...
test_markout_at_horizons_null_when_no_quote_before_target:{[t]
    ...
    r:.qf.markout_at_horizons[trades;quotes;enlist 0D00:00:01];
...
test_markout_at_horizons_crosses_every_trade_with_every_horizon:{[t]
    ...
    r:.qf.markout_at_horizons[trades;quotes;0D00:00:01 0D00:00:10 0D00:01:00];
...
test_markout_at_horizons_works_with_unsorted_quotes:{[t]
    ...
    r:.qf.markout_at_horizons[trades;shuffled_quotes;0D00:00:01];
    .testutil.assertApprox[first r`ref_price;1.1010;1e-9;"correct as-of match even though the input quotes weren't sorted"]};

test_eff_spread_known_buy_above_mid:{[t] .testutil.assertApprox[.qf.eff_spread[1;1.1002;1.1000;10000];4f;1e-6;"buy 0.2 pips above mid -> 2*0.2=0.4... scaled: 4 pip effective spread"]};
test_eff_spread_known_sell_below_mid:{[t] .testutil.assertApprox[.qf.eff_spread[-1;1.0998;1.1000;10000];4f;1e-6;"sell 0.2 pips below mid -> same 4 pip effective spread (symmetric)"]};
test_eff_spread_zero_at_mid:{[t] .testutil.assertApprox[.qf.eff_spread[1;1.1000;1.1000;10000];0f;1e-9;"trade exactly at mid -> zero effective spread"]};
test_eff_spread_negative_is_price_improvement:{[t] .testutil.assertApprox[.qf.eff_spread[1;1.0999;1.1000;10000];-2f;1e-6;"buy below mid -> negative effective spread (price improvement)"]};

test_slippage_known_cost:{[t] .testutil.assertApprox[.qf.slippage[1;1.1000;1.1003;10000];3f;1e-6;"buy fills 3 pips worse than arrival -> +3 pip slippage cost"]};
test_slippage_zero_when_no_move:{[t] .testutil.assertApprox[.qf.slippage[1;1.1000;1.1000;10000];0f;1e-9;"execution at arrival price -> zero slippage"]};
test_slippage_negative_is_improvement:{[t] .testutil.assertApprox[.qf.slippage[1;1.1000;1.0998;10000];-2f;1e-6;"buy fills better than arrival -> negative slippage"]};
test_slippage_sell_side_sign:{[t] .testutil.assertApprox[.qf.slippage[-1;1.1000;1.0997;10000];3f;1e-6;"sell fills 3 pips worse (lower) than arrival -> +3 pip cost"]};

test_fill_ratio_known:{[t] .testutil.assertApprox[.qf.fill_ratio[73;100];0.73;1e-9;"73 fills out of 100 quotes"]};
test_fill_ratio_zero:{[t] .testutil.assertApprox[.qf.fill_ratio[0;100];0f;1e-9;"no fills"]};
test_fill_ratio_full:{[t] .testutil.assertApprox[.qf.fill_ratio[100;100];1f;1e-9;"every quote filled"]};

test_reject_ratio_known:{[t] .testutil.assertApprox[.qf.reject_ratio[4;100];0.04;1e-9;"4 rejects out of 100 requests"]};
...
test_hit_ratio_by_count_mode_grouped_by_sym:{[t]
    ...
    t0:2026.01.01D08:00:00.000000000;
    r:.qf.hit_ratio_by[requests;t0;t0+2D;0Nn;enlist `sym;`count];
...
test_hit_ratio_by_amount_mode_weighted_by_size:{[t]
    requests:mk_hit_ratio_requests[::];
    t0:2026.01.01D08:00:00.000000000;
    r:.qf.hit_ratio_by[requests;t0;t0+2D;0Nn;enlist `sym;`amount];
    eurusd_row:first select from r where sym=`EURUSD;
...
test_hit_ratio_by_no_grouping_gives_one_overall_row:{[t]
    requests:mk_hit_ratio_requests[::];
    t0:2026.01.01D08:00:00.000000000;
    r:.qf.hit_ratio_by[requests;t0;t0+2D;0Nn;`symbol$();`count];
...
test_hit_ratio_by_respects_the_time_window:{[t]
    ...
    requests:mk_hit_ratio_requests[::];
    t0:2026.01.01D08:00:00.000000000;
    r:.qf.hit_ratio_by[requests;t0;t0+0D00:30:00;0Nn;`symbol$();`count];
...
test_hit_ratio_by_hourly_bucket_grouped_by_sym:{[t]
    requests:mk_hit_ratio_requests[::];
    t0:2026.01.01D08:00:00.000000000;
    r:.qf.hit_ratio_by[requests;t0;t0+2D;0D01:00:00;enlist `sym;`count];
...
test_hit_ratio_by_daily_bucket_no_other_grouping:{[t]
    requests:mk_hit_ratio_requests[::];
    t0:2026.01.01D08:00:00.000000000;
    r:.qf.hit_ratio_by[requests;t0;t0+2D;1D;`symbol$();`amount];
...
test_hit_ratio_by_rejects_bad_mode:{[t]
    requests:mk_hit_ratio_requests[::];
    t0:2026.01.01D08:00:00.000000000;
    wrapper:{[q] .qf.hit_ratio_by[q;t0;t0+2D;0Nn;enlist `sym;`bogus]};
...
test_hit_ratio_by_rejects_requests_missing_a_column:{[t]
    ...
    t0:2026.01.01D08:00:00.000000000;
    wrapper:{[q] .qf.hit_ratio_by[q;t0;t0+2D;0Nn;enlist `sym;`amount]};
    .qunit.assertError[wrapper;bad;"a requests table missing a required column is rejected immediately"]};

test_vwap_known_example:{[t] .testutil.assertApprox[.qf.vwap[1.1000 1.1010 1.1005;1000000 2000000 1000000];1.100625;1e-9;"size-weighted average across three fills"]};
test_vwap_single_fill_equals_that_price:{[t] .testutil.assertApprox[.qf.vwap[enlist 1.1234;enlist 1000000];1.1234;1e-9;"a single fill's vwap is just its own price"]};
test_vwap_equal_sizes_equals_simple_average:{[t]
    prices:1.1000 1.1010 1.1020 1.1030;
    sizes:1000000 1000000 1000000 1000000;
    .testutil.assertApprox[.qf.vwap[prices;sizes];avg prices;1e-9;"equal sizes -> vwap reduces to the plain average"]};
...
test_sweep_price_walks_multiple_levels:{[t]
    ...
    r:.qf.sweep_price[prices;sizes;3000000];
...
test_sweep_price_fits_inside_first_level:{[t]
    prices:1.1000 1.1002 1.1005;
    sizes:1000000 1000000 2000000;
    r:.qf.sweep_price[prices;sizes;500000];
...
test_sweep_price_exact_level_boundary:{[t]
    prices:1.1000 1.1002 1.1005;
    sizes:1000000 1000000 2000000;
    r:.qf.sweep_price[prices;sizes;1000000];
...
test_sweep_price_insufficient_liquidity:{[t]
    ...
    prices:1.1000 1.1002 1.1005;
    sizes:1000000 1000000 2000000;
    r:.qf.sweep_price[prices;sizes;5000000];
...
test_sweep_price_of_full_depth_equals_vwap:{[t]
    ...
    r:.qf.sweep_price[prices;sizes;total_size];
    .testutil.assertApprox[r`avg_price;.qf.vwap[prices;sizes];1e-9;"sweeping full depth = vwap of the whole book"]};

test_sweep_price_empty_book_fills_nothing:{[t]
    r:.qf.sweep_price[`float$();`float$();1000000];
...
test_sweep_price_rejects_non_positive_size:{[t]
    wrapper:{[x] .qf.sweep_price[1.10 1.11;100 100;x]};
...
test_sweep_price_rejects_mismatched_lengths:{[t]
    wrapper:{[x] .qf.sweep_price[1.10 1.11;enlist 100;500]};
