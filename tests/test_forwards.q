...
test_fwd_simple_no_differential_is_spot:{[t]
    rs:0.01 0.03 0.07;
    .testutil.assertApprox[.qf.fwd_simple[1.10;rs;rs;1];1.10+0*rs;1e-9;"rd=rf -> forward=spot (simple)"]};

test_fwd_cont_no_differential_is_spot:{[t]
    rs:0.01 0.03 0.07;
    .testutil.assertApprox[.qf.fwd_cont[1.10;rs;rs;1];1.10+0*rs;1e-9;"rd=rf -> forward=spot (continuous)"]};

test_fwd_simple_known_example:{[t] .testutil.assertApprox[.qf.fwd_simple[1.10;0.05;0.02;1];1.132353;1e-5;"EURUSD-style CIRP example, 1y"]};
test_fwd_cont_known_example:{[t] .testutil.assertApprox[.qf.fwd_cont[1.10;0.05;0.02;1];1.10*exp 0.03;1e-9;"continuous CIRP matches exp((rd-rf)*t) directly"]};

test_fwd_points_known:{[t] .testutil.assertApprox[.qf.fwd_points[1.132353;1.10;10000];323.53;1e-1;"forward points in pips for the CIRP example"]};
test_fwd_points_zero_when_no_move:{[t] .testutil.assertApprox[.qf.fwd_points[1.10;1.10;10000];0f;1e-9;"forward equals spot -> zero points"]};

test_points_to_outright_round_trip:{[t]
    fwd:.qf.fwd_simple[1.10;0.05;0.02;1];
    points:.qf.fwd_points[fwd;1.10;10000];
    .testutil.assertApprox[.qf.points_to_outright[1.10;points;10000];fwd;1e-8;"points -> outright round trip"]};

test_implied_foreign_rate_round_trip:{[t]
    spot:1.2500; rd:0.045; rf:0.015; tt:0.5;
    fwd:.qf.fwd_simple[spot;rd;rf;tt];
    .testutil.assertApprox[.qf.implied_foreign_rate[spot;fwd;rd;tt];rf;1e-8;"recovers rf from a forward built with fwd_simple"]};

test_implied_domestic_rate_round_trip:{[t]
    spot:1.2500; rd:0.045; rf:0.015; tt:0.5;
    fwd:.qf.fwd_simple[spot;rd;rf;tt];
    .testutil.assertApprox[.qf.implied_domestic_rate[spot;fwd;rf;tt];rd;1e-8;"recovers rd from a forward built with fwd_simple"]};
...
test_implied_rate_round_trip_across_many_scenarios:{[t]
    ...
    tts:1 0.25 0.5 2;
    fwds:.qf.fwd_simple[spots;rds;rfs;tts];
    .testutil.assertApprox[.qf.implied_foreign_rate[spots;fwds;rds;tts];rfs;1e-6;"implied rf recovered across several currency-pair scenarios"]};

test_cross_rate_triangulation_known:{[t] .testutil.assertApprox[.qf.cross_rate[1.10;150];165f;1e-9;"EURUSD * USDJPY = EURJPY"]};

test_cross_rate_round_trip:{[t]
    ab:1.3427; bc:0.7231;
    ac:.qf.cross_rate[ab;bc];
    .testutil.assertApprox[.qf.cross_rate[ac;.qf.invert_rate bc];ab;1e-9;"(A/B*B/C)*C/B = A/B round trip via invert_rate"]};

test_invert_rate_round_trip:{[t]
    rates:0.5 1.10 150.25 0.7231;
    .testutil.assertApprox[.qf.invert_rate .qf.invert_rate rates;rates;1e-9;"invert twice returns the original rate"]};

test_invert_rate_known:{[t] .testutil.assertApprox[.qf.invert_rate 2f;0.5;1e-9;"1/2=0.5"]};

test_cross_rate_shared_base_known:{[t]
    / EURPLN=4.30, EURUSD=1.075 -> USDPLN=4.30/1.075=4.0 exactly
    .testutil.assertApprox[.qf.cross_rate_shared_base[4.30;1.075];4f;1e-9;"EURPLN, EURUSD -> USDPLN"]};

test_cross_rate_shared_base_matches_manual_composition:{[t]
    rate_ax:1.3427; rate_ay:0.7231;
    lhs:.qf.cross_rate_shared_base[rate_ax;rate_ay];
    rhs:.qf.cross_rate[rate_ax;.qf.invert_rate rate_ay];
...
test_cross_rate_shared_base_identity_when_pairs_equal:{[t]
    ...
    .testutil.assertApprox[.qf.cross_rate_shared_base[1.2500;1.2500];1f;1e-9;"identical shared-base rates cross to exactly 1"]};
...
test_cross_rate_shared_base_anti_symmetric:{[t]
    ...
    fwd:.qf.cross_rate_shared_base[rate_ax;rate_ay];
    back:.qf.cross_rate_shared_base[rate_ay;rate_ax];
...
test_invert_book_round_trip:{[t]
    ...
    back:.qf.invert_book .qf.invert_book book;
...
test_cross_book_direct_form:{[t]
    ...
    r:.qf.cross_book[`EURUSD;eurusd;`USDJPY;usdjpy];
...
test_cross_book_invert_second_leg:{[t]
    ...
    r:.qf.cross_book[`EURUSD;eurusd;`GBPUSD;gbpusd];
...
test_cross_book_invert_first_leg:{[t]
    ...
    r:.qf.cross_book[`USDJPY;usdjpy;`USDCHF;usdchf];
...
test_cross_book_never_crossed_from_valid_inputs:{[t]
    ...
    r1:.qf.cross_book[syms 0;books 0;syms 1;books 1];
    r2:.qf.cross_book[syms 0;books 0;syms 2;books 2];
    r3:.qf.cross_book[syms 1;books 1;syms 3;books 3];
    .qunit.assertFalse[.qf.book_crossed r1;"EUR/USD x USD/JPY synthetic book is not crossed"];
    .qunit.assertFalse[.qf.book_crossed r2;"EUR/USD x GBP/USD synthetic book is not crossed"];
    .qunit.assertFalse[.qf.book_crossed r3;"USD/JPY x USD/CHF synthetic book is not crossed"]};

test_cross_book_rejects_no_shared_currency:{[t]
    wrapper:{[dummy] .qf.cross_book[`EURUSD;`bid`ask!(1.10;1.1002);`GBPCHF;`bid`ask!(1.20;1.2003)]};
    .qunit.assertError[wrapper;::;"EURUSD and GBPCHF share no currency"]};

test_ccy_orient_cross_chain:{[t]
    r:.qf.ccy_orient_cross[`EURUSD;`USDJPY];
...
test_ccy_orient_cross_shared_quote:{[t]
    r:.qf.ccy_orient_cross[`EURUSD;`GBPUSD];
...
test_ccy_orient_cross_shared_base:{[t]
    r:.qf.ccy_orient_cross[`USDJPY;`USDCHF];
...
test_ccy_orient_cross_rejects_no_shared_currency:{[t]
    wrapper:{[dummy] .qf.ccy_orient_cross[`EURUSD;`GBPCHF]};
    .qunit.assertError[wrapper;::;"no shared currency is rejected"]};

test_invert_book_depth_known:{[t]
    r:.qf.invert_book_depth[1.1000 1.1002;1000000 1000000];
...
test_invert_book_depth_round_trip:{[t]
    ...
    once:.qf.invert_book_depth[prices;sizes];
    twice:.qf.invert_book_depth[once 0;once 1];
...
test_cross_book_at_sizes_matches_cross_book_at_negligible_size:{[t]
    ...
    r:.qf.cross_book_at_sizes[`EURUSD;eurusd_book;`USDJPY;usdjpy_book;enlist 100;`bid`ask];
    tob:.qf.cross_book[`EURUSD;`bid`ask!(1.0998;1.1000);`USDJPY;`bid`ask!(149.98;150.00)];
...
test_cross_book_at_sizes_shared_corner_matches_cross_book_at_negligible_size:{[t]
    ...
    r:.qf.cross_book_at_sizes[`AUDUSD;audusd_book;`EURUSD;eurusd_book;enlist 100;`bid`ask];
    tob:.qf.cross_book[`AUDUSD;`bid`ask!(0.6498;0.6500);`EURUSD;`bid`ask!(1.0998;1.1000)];
...
test_cross_book_at_sizes_walks_multiple_levels:{[t]
    eurusd_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(1.0998 1.0996;1000000 1000000;1.1000 1.1002;1000000 1000000);
    usdjpy_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(149.98 149.96;1000000 2000000;150.00 150.02;1000000 2000000);
    r:.qf.cross_book_at_sizes[`EURUSD;eurusd_book;`USDJPY;usdjpy_book;enlist 1500000;`bid`ask`mid];
...
test_cross_book_at_sizes_insufficient_depth:{[t]
    ...
    eurusd_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(1.0998 1.0996;1000000 1000000;1.1000 1.1002;1000000 1000000);
    usdjpy_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(149.98 149.96;1000000 2000000;150.00 150.02;1000000 2000000);
    r:.qf.cross_book_at_sizes[`EURUSD;eurusd_book;`USDJPY;usdjpy_book;enlist 3000000;`bid`ask];
...
test_cross_book_at_sizes_mid_varies_with_asymmetric_depth:{[t]
    ...
    usdjpy_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(149.98 149.96;1000000 2000000;150.00 150.02;1000000 2000000);
    r:.qf.cross_book_at_sizes[`EURUSD;thin_ask_book;`USDJPY;usdjpy_book;500000 3000000;enlist `mid];
...
test_cross_book_at_sizes_sides_filtering:{[t]
    eurusd_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(1.0998 1.0996;1000000 1000000;1.1000 1.1002;1000000 1000000);
    usdjpy_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(149.98 149.96;1000000 2000000;150.00 150.02;1000000 2000000);
    r:.qf.cross_book_at_sizes[`EURUSD;eurusd_book;`USDJPY;usdjpy_book;enlist 1000000;enlist `mid];
...
test_cross_book_at_sizes_rejects_invalid_side:{[t]
    ...
    wrapper:{[books] .qf.cross_book_at_sizes[`EURUSD;books 0;`USDJPY;books 1;enlist 1000000;enlist `close]};
    eurusd_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(1.0998 1.0996;1000000 1000000;1.1000 1.1002;1000000 1000000);
    usdjpy_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(149.98 149.96;1000000 2000000;150.00 150.02;1000000 2000000);
...
test_cross_book_at_sizes_rejects_no_shared_currency:{[t]
    wrapper:{[books] .qf.cross_book_at_sizes[`EURUSD;books 0;`GBPCHF;books 1;enlist 1000000;`bid`ask]};
    eurusd_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(1.0998 1.0996;1000000 1000000;1.1000 1.1002;1000000 1000000);
...
test_ccy_orient_chain_two_legs_matches_ccy_orient_cross:{[t]
    chain:.qf.ccy_orient_chain[`EURUSD`USDJPY];
    pair:.qf.ccy_orient_cross[`EURUSD;`USDJPY];
...
test_ccy_orient_chain_three_legs_forward:{[t]
    r:.qf.ccy_orient_chain[`EURUSD`USDJPY`JPYCHF];
...
test_ccy_orient_chain_three_legs_with_shared_quote_middle_leg:{[t]
    ...
    r:.qf.ccy_orient_chain[`EURUSD`GBPUSD`GBPCHF];
...
test_cross_book_chain_at_sizes_matches_cross_book_at_sizes_for_two_legs:{[t]
    ...
    eurusd_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(1.0998 1.0996;1000000 1000000;1.1000 1.1002;1000000 1000000);
    usdjpy_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(149.98 149.96;1000000 2000000;150.00 150.02;1000000 2000000);
    r_chain:.qf.cross_book_chain_at_sizes[`EURUSD`USDJPY;(eurusd_book;usdjpy_book);enlist 1500000;`bid`ask`mid];
    r_pair:.qf.cross_book_at_sizes[`EURUSD;eurusd_book;`USDJPY;usdjpy_book;enlist 1500000;`bid`ask`mid];
...
test_cross_book_chain_at_sizes_matches_cross_book_at_negligible_size_three_legs:{[t]
    ...
    r:.qf.cross_book_chain_at_sizes[`EURUSD`USDJPY`JPYCHF;(eurusd_book;usdjpy_book;jpychf_book);enlist 100;`bid`ask];
    eurjpy_tob:.qf.cross_book[`EURUSD;`bid`ask!(1.0998;1.1000);`USDJPY;`bid`ask!(149.98;150.00)];
    eurchf_tob:.qf.cross_book[`EURJPY;eurjpy_tob;`JPYCHF;`bid`ask!(0.0065;0.0066)];
...
test_cross_book_chain_at_sizes_shortfall_on_middle_leg_marks_not_fully_filled:{[t]
    ...
    r:.qf.cross_book_chain_at_sizes[`EURUSD`USDJPY`JPYCHF;(big_eurusd_book;thin_usdjpy_book;big_jpychf_book);enlist 1000000;`bid`ask];
...
test_cross_book_chain_at_sizes_recovers_via_deeper_levels_on_thin_bridge_leg:{[t]
    ...
    r:.qf.cross_book_chain_at_sizes[`EURUSD`USDJPY`JPYCHF;(big_eurusd_book;two_level_usdjpy_book;big_jpychf_book);enlist 1000000;`bid`ask];
...
test_cross_book_chain_at_sizes_sides_filtering:{[t]
    ...
    usdjpy_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(149.98 149.96;1000000 2000000;150.00 150.02;1000000 2000000);
    jpychf_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(0.0065 0.0064;5000000 5000000;0.0066 0.0067;5000000 5000000);
    r:.qf.cross_book_chain_at_sizes[`EURUSD`USDJPY`JPYCHF;(eurusd_book;usdjpy_book;jpychf_book);enlist 1000000;enlist `mid];
    .qunit.assertEquals[cols r;`size`sym`mid;"requesting just mid returns only size, sym and mid columns"]};

test_cross_book_chain_at_sizes_rejects_mismatched_syms_and_books:{[t]
    wrapper:{[books] .qf.cross_book_chain_at_sizes[`EURUSD`USDJPY`JPYCHF;books;enlist 1000000;`bid`ask]};
    eurusd_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(1.0998 1.0996;1000000 1000000;1.1000 1.1002;1000000 1000000);
    usdjpy_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(149.98 149.96;1000000 2000000;150.00 150.02;1000000 2000000);
...
test_cross_book_chain_at_sizes_rejects_no_shared_currency:{[t]
    wrapper:{[books] .qf.cross_book_chain_at_sizes[`EURUSD`USDJPY`GBPCHF;books;enlist 1000000;`bid`ask]};
    eurusd_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(1.0998 1.0996;1000000 1000000;1.1000 1.1002;1000000 1000000);
    usdjpy_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(149.98 149.96;1000000 2000000;150.00 150.02;1000000 2000000);
    gbpchf_book:`bid_prices`bid_sizes`ask_prices`ask_sizes!(1.20 1.19;1000000 1000000;1.21 1.22;1000000 1000000);
...
test_ccy_shortest_path_finds_multi_leg_chain:{[t]
    .qunit.assertEquals[.qf.ccy_shortest_path[`AUDUSD`EURUSD`EURPLN;`AUD;`PLN];`AUDUSD`EURUSD`EURPLN;"AUD->PLN needs both bridge legs"]};

test_ccy_shortest_path_prefers_shorter_chain:{[t]
    .qunit.assertEquals[.qf.ccy_shortest_path[`AUDUSD`EURUSD`EURPLN;`USD;`PLN];`EURUSD`EURPLN;"USD->PLN only needs the 2 legs that actually touch USD and PLN, not the AUDUSD leg too"]};

test_ccy_shortest_path_direct_single_leg:{[t]
    .qunit.assertEquals[.qf.ccy_shortest_path[`AUDUSD`EURUSD`EURPLN;`AUD;`USD];enlist `AUDUSD;"currencies already directly connected -> one-leg path"]};

test_ccy_shortest_path_empty_when_unreachable:{[t]
    .qunit.assertEquals[.qf.ccy_shortest_path[`AUDUSD`EURUSD`EURPLN;`AUD;`JPY];`symbol$();"no chain of available pairs connects AUD and JPY"]};

test_ccy_shortest_path_empty_for_same_currency:{[t]
    .qunit.assertEquals[.qf.ccy_shortest_path[`AUDUSD`EURUSD`EURPLN;`USD;`USD];`symbol$();"start and goal the same currency needs no legs at all"]};

test_cross_decomp_three_leg_chain:{[t]
    .qunit.assertEquals[.qf.cross_decomp[`AUDUSD`EURUSD`EURPLN;`AUDPLN];`AUDUSD`EURUSD`EURPLN;"AUDPLN decomposes into the 3 legs bridging AUD->USD->EUR->PLN"]};

test_cross_decomp_two_leg_chain:{[t]
    .qunit.assertEquals[.qf.cross_decomp[`EURUSD`USDRUB;`EURRUB];`EURUSD`USDRUB;"EURRUB decomposes into just the 2 legs bridging EUR->USD->RUB"]};

test_cross_decomp_direct_quote_needs_one_leg:{[t]
    .qunit.assertEquals[.qf.cross_decomp[`AUDUSD`EURUSD`EURPLN;`AUDUSD];enlist `AUDUSD;"a directly-quoted pair decomposes to just itself"]};

test_cross_decomp_accepts_flexible_input_formats:{[t]
    .qunit.assertEquals[.qf.cross_decomp[`AUDUSD`EURUSD`EURPLN;"aud/pln"];`AUDUSD`EURUSD`EURPLN;"lowercase, slash-separated input normalizes the same as `AUDPLN"]};

test_cross_decomp_empty_when_unreachable:{[t]
    .qunit.assertEquals[.qf.cross_decomp[`AUDUSD`EURUSD`EURPLN;`AUDJPY];`symbol$();"no chain of available pairs connects AUD and JPY"]};
...
test_cross_book_at_chains_through_available_pairs:{[t]
    ...
    direct:.qf.cross_book_chain_at_sizes[`AUDUSD`EURUSD`EURPLN;.qf.leg_book_as_of[quotes;2026.01.02D00:00:00.000000000;] each `AUDUSD`EURUSD`EURPLN;enlist 500000;`bid`ask`mid];
    auto:.qf.cross_book_at[quotes;`AUDPLN;2026.01.02D00:00:00.000000000;enlist 500000;`bid`ask`mid];
...
test_cross_book_at_direct_quote_needs_no_chaining:{[t]
    quotes:mk_quotes_table[::];
    r:.qf.cross_book_at[quotes;`AUDUSD;2026.01.02D00:00:00.000000000;enlist 500000;`bid`ask`mid];
...
test_cross_book_at_inverse_of_a_direct_quote:{[t]
    quotes:mk_quotes_table[::];
    r:.qf.cross_book_at[quotes;`USDAUD;2026.01.02D00:00:00.000000000;enlist 500000;`mid];
...
test_cross_book_at_rejects_unreachable_pair:{[t]
    quotes:mk_quotes_table[::];
    wrapper:{[q] .qf.cross_book_at[q;`AUDJPY;2026.01.02D00:00:00.000000000;enlist 500000;`mid]};
...
test_cross_book_at_rejects_quote_after_at_time:{[t]
    quotes:mk_quotes_table[::];
    wrapper:{[q] .qf.cross_book_at[q;`AUDUSD;2025.12.31D00:00:00.000000000;enlist 500000;`mid]};
...
test_cross_book_at_rejects_unsorted_quotes:{[t]
    ...
    wrapper:{[q] .qf.cross_book_at[q;`AUDPLN;2026.01.02D00:00:00.000000000;enlist 500000;`mid]};
...
test_cross_size_at_price_finds_boundary_size:{[t]
    ...
    max_sz:.qf.cross_size_at_price[quotes;`AUDPLN;at_time;`bid;2.5650];
    r_at_max:.qf.cross_book_at[quotes;`AUDPLN;at_time;enlist max_sz;enlist `bid];
    r_above:.qf.cross_book_at[quotes;`AUDPLN;at_time;enlist (max_sz*1.01);enlist `bid];
...
test_cross_size_at_price_rejects_bad_side:{[t]
    quotes:mk_quotes_table[::];
    wrapper:{[q] .qf.cross_size_at_price[q;`AUDPLN;2026.01.02D00:00:00.000000000;`mid;2.5650]};
...
test_cross_size_at_price_near_zero_when_even_negligible_size_breaches:{[t]
    ...
    max_sz:.qf.cross_size_at_price[quotes;`AUDPLN;at_time;`bid;10f];
...
test_cross_markout_at_horizons_negative_horizon_looks_backward:{[t]
    ...
    r:.qf.cross_markout_at_horizons[quotes;`AUDPLN;trade_time;1;2.5600;10000;-500 0 500;1];
...
test_cross_markout_at_horizons_ts_col_is_configurable:{[t]
    ...
    original:.qf.ts_col;
    .qf.ts_col:`timestamp;
    r:.qf.cross_markout_at_horizons[quotes;`AUDPLN;trade_time;1;2.5600;10000;enlist 0;1];
    .qf.ts_col:original;
    ...
    .qunit.assertEquals[cols r;`horizon_ms`timestamp`sym`ref_price`markout_pips;"overriding .qf.ts_col renames the timestamp column and disables col_precedence's reorder (it no longer matches)"]};
...
test_cross_markout_at_horizons_nulls_out_of_range_horizon_instead_of_erroring:{[t]
    ...
    t0:2026.01.01D00:00:00.000000000;
    trade_time:t0+0D00:00:00.500;
    r:.qf.cross_markout_at_horizons[quotes;`AUDPLN;trade_time;1;2.5600;10000;enlist -10000;1];
...
test_cross_markout_decomp_rejects_unreachable_pair:{[t]
    ...
    t0:2026.01.01D00:00:00.000000000;
    t1:t0+0D00:00:01;
    wrapper:{[q] .qf.cross_markout_decomp[q;`AUDJPY;t0;t1;10000;1]};
    .qunit.assertError[wrapper;quotes;"no chain of available pairs connects AUD and JPY"]};
...
test_cross_impact_at_horizons_reports_a_different_pairs_own_drift:{[t]
    ...
    t0:2026.01.01D00:00:00.000000000;
    trade_time:t0+0D00:00:00.500;
    r:.qf.cross_impact_at_horizons[quotes;`EURPLN;`AUDUSD;trade_time;1;10000;-500 0 500;1];
    .qunit.assertEquals[count r;3;"one row per horizon"];
...
test_cross_impact_at_horizons_side_flips_the_sign:{[t]
    ...
    t0:2026.01.01D00:00:00.000000000;
    trade_time:t0+0D00:00:00.500;
    buy_r:.qf.cross_impact_at_horizons[quotes;`EURPLN;`AUDUSD;trade_time;1;10000;enlist 500;1];
    sell_r:.qf.cross_impact_at_horizons[quotes;`EURPLN;`AUDUSD;trade_time;-1;10000;enlist 500;1];
...
test_cross_impact_at_horizons_rejects_same_pair:{[t]
    ...
    t0:2026.01.01D00:00:00.000000000;
    trade_time:t0+0D00:00:00.500;
    wrapper:{[q] .qf.cross_impact_at_horizons[q;`EURPLN;`EURPLN;trade_time;1;10000;enlist 0;1]};
...
test_cross_impact_at_horizons_sym_column_is_impact_sym_not_traded_sym:{[t]
    ...
    t0:2026.01.01D00:00:00.000000000;
    trade_time:t0+0D00:00:00.500;
    r:.qf.cross_impact_at_horizons[quotes;`EURPLN;`AUDUSD;trade_time;1;10000;enlist 0;1];
...
test_cross_book_at_rejects_quotes_missing_a_column:{[t]
    ...
    wrapper:{[q] .qf.cross_book_at[q;`AUDUSD;2026.01.02D00:00:00.000000000;enlist 500000;`mid]};
...
test_cross_markout_at_horizons_rejects_quotes_missing_a_column_instead_of_nulling:{[t]
    ...
    bad:delete ask_prices from quotes;
    trade_time:2026.01.01D00:00:00.000000000+0D00:00:00.500;
    wrapper:{[q] .qf.cross_markout_at_horizons[q;`AUDPLN;trade_time;1;2.5650;10000;enlist 0;1]};
...
test_cross_markout_decomp_rejects_quotes_missing_a_column:{[t]
    ...
    t0:2026.01.01D00:00:00.000000000;
    t1:t0+0D00:00:01;
    wrapper:{[q] .qf.cross_markout_decomp[q;`AUDPLN;t0;t1;10000;1]};
    .qunit.assertError[wrapper;bad;"a quotes table missing a required column is rejected immediately"]};
...
test_apply_col_precedence_leaves_table_unchanged_when_precedence_not_fully_present:{[t]
    ...
    .qunit.assertEquals[.qf.apply_col_precedence t;t;"a table with sym but no ts column is left completely unchanged"]};
