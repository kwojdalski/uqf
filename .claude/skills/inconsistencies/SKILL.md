...
- A new module (`src/*.q`) that doesn't follow the established `\d .qmodule` ... `\d .` wrapping pattern, or that nests namespaces more than one level deep — except the ETL tree's instances, which are deliberately `\d .qwrk.<worker name>` (derived by `.qbw.define`) and `\d .qfeed.<source name>`
