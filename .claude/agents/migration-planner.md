...
- **`docs/migrations/`** — read any prior plan there before writing a new one. If a previous plan was partly executed, your job is to plan from the real current state, not from its starting state. The directory is EMPTY as of 2026-09-24 and does not exist on disk: its two plans both targeted a canonical upstream that has since been frozen, so they planned a migration that will never happen and were deleted. Create the directory when you write the first new plan; finding nothing there means there is nothing, not that you looked in the wrong place.
