...
- Don't introduce a new namespace nested more than one level deep (`` \d .qfwd.sub ``) — every namespace in the quant library is flat by convention (N-01). (The ETL tree nests its bounded-worker instances under `.qwrk`; that is the one exception, and it is not yours.)
