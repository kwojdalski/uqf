...
- Don't introduce a new namespace nested more than one level deep (`` \d .qfwd.sub ``) — every namespace in the quant library is flat by convention (N-01). (The ETL tree nests its instances — workers under `.qwrk`, sources under `.qfeed`; those are the exceptions, and they are not yours.)
