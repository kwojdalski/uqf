/ demo_deals_backfill.q - the demo deals bounded worker (.qpipe.job.demo_deals_backfill).
/ .
...
/ other 234 were glue every bounded worker needs, and are now in .qetl.job.bounded once.
/ .
...
/ schema, which only .qetl.source.validate_live on the work machine can settle.
/ .
/ WHERE THE CONTRACT'S NAMES ARE
/ .
/ In this namespace, stamped by the define call at the bottom (#227):
/ The contract's globals and the delegating methods are written here by .qetl.job.bounded
...
\d .qpipe.job.demo_deals_backfill
...
/ @eg .qpipe.job.demo_deals_backfill.quality_check[.qpipe.source.demo_deals.fixture[]]  ->  an empty table
quality_check:{[batch]
...
.qetl.transform.passthrough[`demo_deals_passthrough;`batch;0#.qpipe.source.demo_deals.fixture[];.qpipe.source.demo_deals.fixture[]];
