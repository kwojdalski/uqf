/ market_data.q - direct FX source snapshots in one shape (.qsub.market_data).
/ .
...
publish:.qstream.unwired `market_data;
...
/ @eg count .qsub.market_data.from_quote[.qsub.market_data.quote] -> 0
from_quote:{[batch]
...
/ @eg count .qsub.market_data.from_quotes[.qsub.market_data.quotes] -> 0
from_quotes:{[batch]
    fx:batch where .qccy.is_ccy_pair each batch`sym;
...
.qxf.define[`market_data_from_quote;`inputs`output`fn`examples!(
    (enlist `quote)!enlist .qsub.market_data.quote;
    .qsub.market_data.market_data;
    .qsub.market_data.from_quote;
...
.qxf.define[`market_data_from_quotes;`inputs`output`fn`examples!(
    (enlist `quotes)!enlist .qsub.market_data.quotes;
    .qsub.market_data.market_data;
    .qsub.market_data.from_quotes;
    enlist `inputs`expected!(
...
.qnorm.define[`market_data;`procname`output`sources!(
    `marketdata1;
    .qsub.market_data.market_data;
