...
.qstream.register[`cross;`procname`subscribes`publishes`on_batch`note!(
    ...
    `symbol$();
    .qsub.cross.on_batch;
    "keeps cross_quotes as private process state, publishes no table - so it is a leaf, and nothing downstream stalls while it is stopped. startwithall:0 to stay inside PLANT_CONNECTION_BUDGET (#285); quotesfeed1 runs by default, so `uqf-stack start cross1` is enough")];
