...
**23 vendored processes** plus **26 uqf processes** — 49 in total. Ports are shown at the default base port 6050; every one is `{KDBBASEPORT}+offset`, so a different base shifts them all together.
