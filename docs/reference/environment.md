...
- **One variable is built mechanically and appears nowhere in the source.**
  `.qetl.cfg.env_name` maps a config key to `UQF_` plus the upper-cased key, so
  `UQF_DRY_RUN` is read by `.qetl.job.bounded.runtime.is_dry_run` without the
  string `UQF_DRY_RUN` existing in any production file. `grep` cannot find it.
  That is the single strongest argument for this page existing.
...
## Read by this repository
  ...
  | ---                             | ---                                                                                     | ---                            | ---                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
  | `UQF_DRY_RUN`                   | `.qetl.job.bounded.runtime.is_dry_run` (`src/etl/core/worker_runtime.q`)                | no                             | false — the worker publishes for real. Opt-in on purpose: defaulting to true would make a worker silently do nothing and report success                                                                                                                                                                                                                                                                                                                                                     |
  | `UQF_SOURCE_CRED_<SOURCE>`      | `.qetl.source.require_credentials` (`src/etl/core/source_contract.q`)                   | per live source                | `require_credentials` refuses and names the variable. There is deliberately no file and no vault fallback                                                                                                                                                                                                                                                                                                                                                                                   |
  ...
  | `UQFSTATUSDIR`                  | `.qetl.status.status_dir` (`src/etl/core/status.q`)                                     | no                             | falls back to `$TORQDATA/status`. Pairs with `UQF_FRONTEND_STATUS_DIR` on the reading side                                                                                                                                                                                                                                                                                                                                                                                                  |
...
the `.qetl.cfg` precedence fixtures) are deliberately absent: they are not an
