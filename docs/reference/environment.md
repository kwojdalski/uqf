...
## Read by this repository
  ...
  | `QCMD`                          | `uqs.paths.q_interpreter`, `scripts/test.py`                   | no                             | `~/.kx/bin/q`. The one q interpreter: every q lane, the running stack (`build_env` passes it to `torq.sh`), its HDB filler and the backfill launcher. Together with `QHOME` this is the deliberate, explicit way to point the tree at another q — there is no automatic fallback, see [the README](../../README.md#requirements). It replaced `Q`, `QBIN`, `UQFQ` and the stack's bare `q` on `PATH` (#414)                                                                                 |
...
about where data lives, which is a confusing way to spend an afternoon.
  ...
  | `RLWRAP`        | `rlwrap`                                       |
  | `QCON`          | `qcon`                                         |
  | `QCMD`          | `$QCMD`, else `~/.kx/bin/q` --- see above      |
...
for what else may work). Nothing falls back automatically: if `QHOME` and
`$QCMD` point somewhere the tree is not verified against, that is the operator's
