...
| `UQF_FRONTEND_CAPTURE_DIR` | as above | no | where usage rows are captured to, as JSON Lines (FE-13). **Unset means capture does not run**, and that is lossy rather than merely inactive: `.usage.flushtime` is three hours, so without it the usage view can only ever show the last three hours and older history is gone for good |
