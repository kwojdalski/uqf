...
inputs change. Nothing can observe a change as it happens.
...
assignment, and this stack is operated by ad-hoc IPC where a plain assignment is
the common case. It would audit exactly the changes made by people who did not
need auditing.
...
Not in this table, and deliberately not --- because the answer already exists.
TorQ's `logusage.q` writes every incoming IPC command per process, with the
...
timestamp and you have both halves. Neither can answer the question alone: the
usage log does not know whether a command *changed* anything, and the audit
table does not know who was on the handle.
...
judgement each reader makes again. Names must be fully qualified --- a bare
`` `notional `` resolves against whatever namespace is current when the timer
fires, which is not the one the author meant, so it is refused.
...
already installs timers, so this is a second timer publishing through the seam
that was there.
