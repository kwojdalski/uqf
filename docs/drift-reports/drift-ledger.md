...
Of the three rows that looked actionable when this ledger was first written,
only one survived inspection, and that is the ledger's most useful output:
...
It stops "narrow the drift" from meaning "make the number go down". Two
corollaries, both learned the hard way in this file:

- **A row is only closed by a check that can be re-run.** The "Closed by"
  cell names the check, not the work — so a closed row can regress and be
  caught, rather than standing on a claim nobody can test again.
- **The narrative must not outlive the table.** An earlier version of this
  section said "`open` is zero again" while D9 was open, and listed D7 as
  waiting on a packaging decision two paragraphs after declaring it closed.
  The counters and this prose are what people read; when they disagree with
  the table, the table is right and the ledger has lost the credibility that
  is its entire value.
