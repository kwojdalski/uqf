...
**`on_batch` takes two arguments**, and the first is the one that catches people
out. `t` is the table name the batch arrived on --- a symbol, not the data ---
...
why `` `time _ batch `` appears in this tree's history as a bug worth naming: it
...
**An empty batch is legal.** Publishing nothing for a tick is normal, and every
aggregate has to survive it --- `0=count data` on the way in is cheaper than a
null propagating into a P&L number.
