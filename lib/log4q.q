/ log4q.q - vendored from prodrive11/log4q, a concise logger for q/kdb+
/ applications (severity levels, multiple sinks - stdout/stderr/file/TCP,
/ runtime-switchable pattern layouts, printf-style message formatting).
/ Source: https://github.com/prodrive11/log4q/blob/master/log4q.q
/ @author p.bukowinski@gmail.com
/ License: Apache License 2.0 (see lib/LICENSE-log4q) - distributed under
/ its own license, not this repository's MIT license; see README's
/ Licensing section.
/ One customization from upstream: the default `fm` pattern layout below
/ drops %h (hostname - not useful for local dev/example scripts) and the
/ nanosecond-precision %p timestamp in favour of %d/%t (date/time, %t is
/ already millisecond-precision) - upstream's default was
/ "%c\t[%p]:H=%h:PID[%i]:%d:%t:%f: %m\r\n". Still callable/reconfigurable
/ at runtime like any other log4q setting (.log4q.fm:"...").
/ .
\d .log4q
fm:"%c\t[%d %t]:PID[%i]:%f: %m\r\n";
