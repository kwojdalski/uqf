...
`subscribes`/`publishes` fields on each `Pipeline`, the `.qdag` registry in
...
from torq_orchestrator.model.plant_schema import _publishers
from torq_orchestrator.model.registry import PIPELINES
...
EXTERNAL_PRODUCERS: dict[str, str] = {
    "databento_mbp10": (
        "the external Databento feed handler (`uqf-stack databento-feed start`) "
...
def dependency_rows(pipelines: Iterable[Any] = PIPELINES) -> list[dict[str, str]]:
    ...
    The listing behind `uqf-stack list dependencies`. Sorted by consumer so
...
def unfed_inputs(
    ...
    `uqf-stack start marketdata1 superbook1 arbitrage1` is silent rather
    ...
    for table in inputs_by_process(pipelines).get(procname, ()):
        ...
        if declared:
            ...
            line = (
                ...
                f"publishes - start `uqf-stack start {names}`"
            )
            lines.append(
