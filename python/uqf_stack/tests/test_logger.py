...
`uqf-stack logs -f`, whose subprocess-and-thread plumbing had never run.
...
from uqf_stack.logger import core as logcore
from uqf_stack.stack import logs
...
def test_configure_logging_writes_a_dated_per_component_file(tmp_path):
    logcore.configure_logging(component="uqf_stack", log_dir=str(tmp_path), include_console=False)
    logger.info("hello")
    logger.remove()
    files = list(tmp_path.glob("uqf_stack_*.log"))
...
def test_follow_logs_streams_a_line_appended_after_it_starts(tmp_path, monkeypatch):
    """The live tail behind `uqf-stack logs -f`: one `tail -F` per file,
...
def test_follow_logs_refuses_when_there_is_nothing_to_follow(tmp_path, monkeypatch):
    ...
    with pytest.raises(logs.UqfStackError, match="has the demo been started"):
        logs.follow_logs(paths, "rdb1")
