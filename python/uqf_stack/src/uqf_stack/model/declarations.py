...
def _call_body(source: str, open_at: int) -> str:
    ...
    while i < len(source):
        ...
        i += 1
    raise UqfStackError("unterminated declaration call")
...
def _boolean(value: str, where: str) -> bool:
    if value not in ("1b", "0b"):
        raise UqfStackError(f"{where}: autostart must be 1b or 0b, not {value!r}")
    return value == "1b"


def _string(value: str, where: str) -> str:
    if not (len(value) >= 2 and value.startswith('"') and value.endswith('"')):
        raise UqfStackError(f"{where}: note must be a q string literal, not {value!r}")
    escapes = {"n": "\n", "t": "\t"}
...
def _declaration(fn: str, name: str, fields: dict[str, str], path: Path) -> Declaration:
    ...
    if fn == "qbw.define":
        if "autostart" in fields:
            raise UqfStackError(f"{where}: a bounded worker never starts with the stack")
    ...
    if not proc:
        raise UqfStackError(f"{where}: declares no procname")
