...
- **Half-open intervals** - every coverage interval is half-open ``[range_from, range_to)``,
...
- **Required source_version** - ``source_version`` is an immutable source-release label and
  coverage consumers **must filter on it**. Coverage recorded under one
  release says nothing about another.
- **No merging across versions** - intervals from different versions are never merged to satisfy a
  dependency.

This module is deliberately pure so the interval logic is unit-testable
without a gateway, which is the same pure/impure split the q side makes.
"""
...
def compose(intervals: list[Interval]) -> list[Interval]:
    ...
    Composing only at a common boundary is the half-open rule, and it is the whole
...
def from_rows(
    ...
    by the interval rule, so encountering one here means the ledger itself is damaged. It
...
def _aware(value: dt.datetime) -> dt.datetime:
    """Treat a naive timestamp from q as UTC - everything is UTC internally.
