...
`uqs schema` answers "what tables are in the database, and what shape
are they" without anyone hand-writing `meta` at a q prompt.

**It reads the LIVE database, not the declarations.** `scripts/processes/uqs_tables.q`
...
def resolve_port(paths: UqsPaths, procname: str, base_port: int = DEFAULT_BASE_PORT) -> int:
    ...
    raise UqsError(f"{procname!r} is not a declared process - known: {known}")
