...
def _deferred():
    return [p for p in PIPELINES if p.subscribe_to is FROM_DECLARATION]
...
def test_a_spelled_out_edge_is_left_alone():
    ...
    spelled = replace(_deferred()[0], subscribe_to=("a", "b"), publishes=("c",))
...
def test_publishes_none_still_falls_back_to_the_owned_table():
    ...
    pipeline = replace(_deferred()[0], subscribe_to=(), publishes=None, table="one_table")
    assert resolve_edges(pipeline)[1] == ("one_table",)


def test_a_pipeline_with_no_table_and_no_publishes_publishes_nothing():
    pipeline = replace(_deferred()[0], subscribe_to=(), publishes=None, table=None)
