...
`docs/architecture/stack.md` listed uqf's processes in prose and had already lost
...
GENERATED = REPO / "docs" / "reference" / "processes.md"
...
def test_the_vendored_count_is_read_not_remembered():
    """`docs/architecture/stack.md` says "the vendored 14-process stack" in prose.
...
def test_the_prose_architecture_doc_is_consistent_with_the_registry():
    """`docs/architecture/stack.md` is authored — diagrams and explanation — but the
    ...
    """
    readme = (REPO / "docs" / "architecture" / "stack.md").read_text()
    missing = [p.procname for p in PIPELINES if not re.search(rf"\b{p.procname}\b", readme)]
    assert not missing, (
        f"docs/architecture/stack.md does not mention {missing}; it is "
