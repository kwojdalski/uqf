...
def repo(tmp_path: Path) -> Path:
    ...
    (root / STREAM_DIR / "quotes.q").write_text(
        ".qstream.register[`quotes;`procname`publishes!(`quotes1;enlist `quote)];\n"
    )
...
def test_classify_reads_the_declaration_not_the_filename() -> None:
    ...
    assert install.classify("/ .qsrc.register[x;y]\n") == (set(), ())
