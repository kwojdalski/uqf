...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "lib/torq-finance-starter-pack) bridged by uqf's uqs tooling. "
        "Start the stack with uqs_start before querying it, and stop "
        "it with uqs_stop when done - it leaves real q processes "
...
def uqs_query(
    ...
    + 2). See docs/guides/uqs.md's port table for every process's offset.
...
@mcp.tool
def uqs_set_config(procname: str, field: str, value: str) -> str:
...
@mcp.tool
def uqs_list_kinds() -> list[str]:
    """List the kinds uqs_list accepts - not just processes."""
...
def uqs_list(kind: str = "processes", port: int = DEFAULT_BASE_PORT) -> Any:
    ...
    valid uqs_set_config columns; 'overrides' lists every
    uqs_set_config override currently in effect; 'env' lists
...
@mcp.tool
def uqs_crypto_stop() -> str:
    """Stop the cryptorust recorder started by uqs_crypto_start."""
    try:
...
@mcp.tool
def uqs_crypto_status() -> dict[str, str]:
...
def uqs_crypto_fills_start(
    ...
    one itself, unlike uqs_crypto_start which owns its own exchange
    connectors.
    """
    try:
...
@mcp.tool
def uqs_crypto_fills_stop() -> str:
    """Stop the cryptorust fills recorder started by
    uqs_crypto_fills_start.
    """
    try:
...
@mcp.tool
def uqs_crypto_fills_status() -> dict[str, str]:
