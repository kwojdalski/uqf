#!/usr/bin/env python3
"""Derive `docs/reference/processes.md` from the pipeline registry.
...
`docs/architecture/stack.md` listed uqf's processes in prose and had already gone
...
`subscribeto`/`publishes` are declarations, and a declaration can lie. They
...
def _vendored_procnames() -> list[str]:
    ...
    Read rather than remembered: `docs/architecture/stack.md` says "the vendored
