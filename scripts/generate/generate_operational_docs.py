...
def main() -> int:
    ...
    # Two outputs, one gate. Both derive from PIPELINES, so checking only one
    # would let the other drift - and the q bridge is the one a running
    # process depends on.
    outputs = ((OUT, render()), (DAG_OUT, render_dag()))
