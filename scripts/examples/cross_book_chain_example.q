...
mk_timestamps:{[n;start_ts]
    ...
    p:1e-9+(1-2e-9)*n?1.0;
    .qetl.log.dbg[`cross_book_chain;"running: .qstats.inv_ncdf p";()!()];
    z:.qstats.inv_ncdf p;
...
.qetl.log.info[`cross_book_chain;"quotes - ",.Q.s1[count quotes]," rows, one 10-level book per pair, no wide-table reshape needed for this example";()!()];
show quotes;
...
if[not all audpln_book`bid_fully_filled;
    .qetl.log.err[`cross_book_chain;"at least one requested size did not fully fill on the bid side - unexpected for this synthetic depth";()!()];
    exit 1];
if[not all audpln_book`ask_fully_filled;
    .qetl.log.err[`cross_book_chain;"at least one requested size did not fully fill on the ask side - unexpected for this synthetic depth";()!()];
    exit 1];
...
if[not manual_audpln_book~audpln_book;
    .qetl.log.err[`cross_book_chain;"manual chain result disagrees with cross_book_at - the two paths should always match";()!()];
    exit 1];
...
.qetl.log.info[`cross_book_chain;"audusd_ticks - ",.Q.s1[count audusd_ticks]," successive AUDUSD quotes, 10ms apart, price drifting up:";()!()];
show audusd_ticks;
...
if[not mid_at_tick1[`mid]~mid_between`mid;
    .qetl.log.err[`cross_book_chain;"querying between tick1 and tick2 should still return tick1's price - got a different value, as_of is peeking ahead";()!()];
    exit 1];
if[mid_at_tick1[`mid]~mid_at_tick3`mid;
    .qetl.log.err[`cross_book_chain;"querying at tick3 should return a later, different price than tick1 - got the same value, as_of isn't picking up new ticks";()!()];
    exit 1];
.qetl.log.info[`cross_book_chain;"confirmed: between-tick query matches the earlier tick exactly, and the tick3 query differs from it - as_of correctly reflects only information known as of that instant";()!()];
...
if[10h<>type caught;
    .qetl.log.err[`cross_book_chain;"querying before any tick existed should have thrown an error, not returned a result";()!()];
    exit 1];
