#!/usr/bin/env python3
"""Derive ``docs/decisions.md`` from the GitHub ``decision`` issues.
...
Usage::
    ...
    python3 scripts/build_decision_log.py            # rewrite docs/decisions.md
...
OUT = REPO_ROOT / "docs" / "decisions.md"
...
def render(bank: Bank) -> str:
    ...
    for i, a in sorted(answered.items()):
        ...
        add(f"| `{i}` | {area} | {a.headline} | [#{a.issue}](../../issues/{a.issue}) | {a.date} |")
    add("")
...
def main() -> int:
    ...
    ap.add_argument(
        "--check",
        action="store_true",
        help="exit 1 if docs/decisions.md differs from what GitHub says",
    )
    args = ap.parse_args()

    text = render(parse(fetch_issues()))
    if args.check:
        current = OUT.read_text(encoding="utf-8") if OUT.exists() else ""
        if current != text:
            print(
                f"{OUT.relative_to(REPO_ROOT)} is stale - rerun scripts/build_decision_log.py",
                file=sys.stderr,
            )
            return 1
        print(f"{OUT.relative_to(REPO_ROOT)} matches GitHub")
        return 0
